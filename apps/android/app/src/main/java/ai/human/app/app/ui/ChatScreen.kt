...
import ai.human.app.GatewayManager
...
fun ChatScreen(gatewayManager: GatewayManager) {
    ...
    val expressiveDampingRatio = remember {
        HUTokens.springExpressiveDamping / (2 * sqrt(HUTokens.springExpressiveStiffness * HUTokens.springExpressiveMass))
    }
    ...
    Column(
        ...
        Row(
            modifier = Modifier
                .fillMaxWidth()
                .padding(bottom = HUTokens.spaceXs),
        ...
        errorBanner?.let { msg ->
            Card(
                ...
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(bottom = HUTokens.spaceSm)
            ) {
                Row(
                    modifier = Modifier
                        .fillMaxWidth()
        ...
        LazyColumn(
            ...
            itemsIndexed(messages, key = { index, _ -> "msg_$index" }) { _, msg ->
                AnimatedVisibility(
                    visible = true,
                    enter = fadeIn(animationSpec = spring(dampingRatio = expressiveDampingRatio, stiffness = HUTokens.springExpressiveStiffness)) +
                        slideInVertically(animationSpec = spring(dampingRatio = expressiveDampingRatio, stiffness = HUTokens.springExpressiveStiffness), initialOffsetY = { it }),
                    exit = fadeOut(animationSpec = spring(dampingRatio = expressiveDampingRatio, stiffness = HUTokens.springExpressiveStiffness)) +
                        slideOutVertically(animationSpec = spring(dampingRatio = expressiveDampingRatio, stiffness = HUTokens.springExpressiveStiffness), targetOffsetY = { it })
                ) {
                    ChatBubble(
                        text = msg.text,
                        role = msg.role
        ...
        Row(
            ...
            OutlinedTextField(
                ...
                modifier = Modifier
                    .weight(1f)
                    .padding(end = HUTokens.spaceSm),
                placeholder = { Text("Message") },
                textStyle = MaterialTheme.typography.bodyLarge,
                singleLine = true,
                shape = RoundedCornerShape(HUTokens.radiusLg)
            )
            IconButton(
                onClick = {
...
private fun ChatBubble(
    ...
    Row(
        ...
        Card(
            ...
            shape = RoundedCornerShape(HUTokens.radiusXl)
        ) {
            Text(
                text = text,
                modifier = Modifier.padding(HUTokens.spaceMd),
...
private fun ToolCallCard(
    ...
    Card(
        ...
        shape = RoundedCornerShape(HUTokens.radiusLg)
            ...
            Column(modifier = Modifier.padding(HUTokens.spaceMd)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text(name, style = MaterialTheme.typography.titleSmall)
                Spacer(modifier = Modifier.weight(1f))
                if (status == ToolStatus.RUNNING) {
                    CircularProgressIndicator(
                        modifier = Modifier.height(HUTokens.spaceMd),
                        strokeWidth = HUTokens.radiusSm
                    )
                }
            }
