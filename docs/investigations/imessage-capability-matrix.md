...
| **Edit message (send)** | 1 in 50 messages | IMCore dylib (SIP-off) — selector VERIFIED present on macOS 26 | Not implemented | — | ⚠️ **2026-07-21 correction:** the "broken on Tahoe / NO-GO" verdict below was a STALE-SIGNATURE false negative, not a real OS gap. imsg probes `editMessage:…backwardCompatabilityText:` and `editMessageItem:…backwardCompatabilityText:` — **neither exists on macOS 26**, so `imsg status` reports `editMessage ✗`. The current selector `-[IMChat editMessageItem:atPartIndex:withNewPartText:newPartTranslation:backwardCompatabilityText:]` (note the inserted `newPartTranslation:`) **IS present** (verified via `class_getInstanceMethod`, sig `v56@0:8@16q24@32@40@48`). Fix being contributed upstream to imsg. NOTE: existence verified, BEHAVIOR not yet (needs a dylib prototype that actually edits a message). |
...
| **Music preview send (30s)** | Common | iTunes/Spotify API + AppleScript attachment | **Implemented** | — | iTunes .m4a preview + optional album art + Spotify/Apple Music rich link (auto-detected from user preference). Taste learning tracks reactions with persistent storage (~/.human/music_taste.json). Spotify token cached with expiry. See `src/multimodal/music.c`. |
...
| --- | --- | --- | --- |
| Edit messages | `imessage-edit-feasibility.md` (2026-03-08) | Not feasible via public APIs | **2026-07-21 UPDATE:** the "broken on Tahoe / NO-GO" was a stale-signature false negative. `-[IMChat editMessageItem:atPartIndex:withNewPartText:newPartTranslation:backwardCompatabilityText:]` IS present on macOS 26 (verified); imsg calls the pre-`newPartTranslation:` signatures, which were removed. SIP-off + dylib still required (private API). Behavior not yet proven. See bb-events-schema.md §8.7-8.10. |
