...
| 11 | `src/config/config.c` + `include/human/config.h` | **modify** | +35 | New config block `kv_compression { enabled, codec, rank, clip, window, sink_tokens, allow_s2 }`. Parsed and validated; absent config → disabled. |
