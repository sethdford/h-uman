...
| **O3** | — | **attribution LANDED + deployed (b6fd7965f); root cause found AFTER, and it is one contact** | **Third correction (2026-09-20 evening, from live data):** 111 of 122 fired proposals in the log — **91%** — target `+18018983303`, which `blue_guard` holds every time as *not iMessage-reachable* (`whois=0`; remedy `HU_IMESSAGE_ALLOW_GREEN=1`). 93 of those 111 left no outcome row (the throttle silently ate the repeats) and 18 were held. So the eval's "89 dropped pre-send" was almost entirely one number, and FIR is starved not because gates lacked attribution but because the proposer spends its budget on a contact that cannot receive. The carve's attribution ran end-to-end on the first live tick (a proposal cleared all 8 gates, channel held it) and will now label the throttle drops — useful, but not the fix. **The fix is upstream: exclude channel-unreachable contacts from the candidate list.** It cannot live in `init_proposer.c` (agent-core boundary: no channel knowledge) — it goes daemon-side, pre-proposer, on the chat.db routing inference `blue_guard` already uses (not a per-tick `imsg whois` subprocess). Changes who gets texted → OFF→SHADOW→LIVE | SHADOW logs `would-exclude <contact> (unreachable)` + count; LIVE only when FIR reaches **n ≥ 30** with FIR not worse; `fires per contact` no longer dominated by one number |
...
- 2026-09-20 (evening) — O3 root cause from live data: 91% of fires go to one blue_guard-held contact; fix is a daemon-side reachability pre-filter, scoped not built.
- 2026-09-20 (later) — O3 landed via the carve; see the O3 row for the coverage caveat.
