...
| **O3** | — | **LANDED (code); unverified in prod** | The 89 `dropped_pre_send` fires now leave a `proactive_send`/`decline` row naming the gate — `llm_skip`, `protective_boundary`, `governor_gated`, `reactive_recent`, `validator_emptied`, `rate_limited`, `send_cap`, `sanitize_refused`. Landed by carving the gate chain + send (daemon.c 1566–1668) into `hu_daemon_proactive_gate_and_send`; `daemon.c` 10574→10478, which also took it off the file-size ceiling (largest is now `agent_turn.c` 10512). Original misdiagnosis kept in the Correction section above. **The proactive tick is compiled out under HU_IS_TEST** (`daemon_housekeeping.c:73`), so the suite proves the carve compiles and regressed nothing measured — it exercises none of the moved code. A direct test drives the `SKIP` gate → row; the rest is proven only by a normalized diff against the removed text (residual = the intended edits). Real proof is a decline row from live traffic after deploy | `SELECT reason, COUNT(*) FROM proactive_decisions WHERE trigger='proactive_send' AND decision='decline' GROUP BY reason` shows the gates; `fir_dropped_pre_send` → 0 on the next eval run |
