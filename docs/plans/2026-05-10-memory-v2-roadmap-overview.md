...
**W7 — Memory facade.** Introduces `hu_memory_facade_t` (`hu_memory_facade_read` / `hu_memory_facade_write` / … in `human/memory/memory.h`) as the single entry point for structured memory kinds. Implements `hu_memory_v1_backend_t` that wraps existing graph, vector projections, persona deltas, cross_edges, cases, quarantine. Agent/persona/feeds sources no longer spell **`hu_graph_*`** at call sites: they use the facade, **`struct hu_graph *`** parameters where a raw v1 handle is still required, and thin **`hu_memory_*`** shims in `memory.h` (see [`2026-05-10-w7-phase1-bypass-inventory.md`](2026-05-10-w7-phase1-bypass-inventory.md); scoped inventory script reports **0**). Channels and deeper read/write hot-path work continue in phased passes per the execution plan. The facade dispatches; backends do the work. This is the foundation that lets later workstreams swap memory backends (KV-cache, activations, etc.) without rewriting consumers.
