...
| 5 | ✅ 2026-05-13 ([phase-5 plan](2026-05-11-rl-loop-phase-5-eval-competitive.md)) — critic + spec-verifier (BLOCKER-1/-2/-3, NEW-1/-2 HIGH, NEW-MED-1/-2/-3 all addressed before any code; folds in 3 Phase 2 production-wiring deferrals) | ✅ 2026-05-13 | ✅ 2026-05-16 (tag `rl-sota-phase-5-complete` @ `a16cb489`) | ✅ PASS — full rl_sota suite green under ASan, 0 leaks; 4-axis fidelity scorer v2 (`hu_communication_style_fidelity_score_v2` — adds decision-style axis additively on top of v1, v1 untouched); bootstrap-CI helper (`src/eval/bootstrap_ci.c`, resampling-with-replacement on score vector, `n_responses ≥ 30` floor in production / `≥ 10` in tests); `hu_eval_gate_t` (composes scorer + leaderboard + reward model + latency with codified NULL-skip semantics for `mt_bench`/`ifeval`/`reward_model`); external judge vtable (`hu_eval_judge_external_t` with canned + Apple FM Swift FFI + Gemini Nano headless-Chrome stubs); competitive harness orchestrating side-by-side eval with v2 scorer; `human eval gate` / `human eval competitive` / `human eval leaderboard` CLIs (gated behind `HU_ENABLE_RL_FULL`); production wiring of the three Phase 2 sprint-auditor deferrals: (a) **eval-gate-before-`hu_provider_load_adapter`** in `src/agent/lora_training_runner.c::hu_lora_training_runner` — structurally wired (gate called via `hu_eval_gate_decide_from_arrays_for_test` before adapter load; verified by `tests/test_lora_training_runner_eval_gate.c`), **but fed hard-coded synthetic inputs today** (`persona[20] = {0.75…}`, other arrays NULL, `p95=100.0`) — real measured metrics is carry-forward **CF-4**; (b) **`gate_decision.json` evidence directory contract** per spec §8 — file schema honored, but five of nine files are 3-byte `{}` stubs and `reproduce.sh` is a 25-byte placeholder, `manifest.json::persona_delta=0.06` and `gate_decision.json` body are hard-coded literals in `src/ml/cli_demo.c` — carry-forward **CF-2**; (c) **`hu_imessage_poll_reactions` wired into the daemon poll loop** — landed only as `#if HU_IS_TEST` scaffolding in `src/daemon/daemon_reaction_poll.c`, **not** as production daemon code; `hu_reaction_handler_set_collector` is never called from `src/daemon.c` — carry-forward **CF-3**. The close-out sprint-auditor at `010763ef` caught the original "all three closed" framing as fabricated; the honest "structural wiring landed, full production wiring is open carry-forward" framing is corrected here. See [`docs/proof/rl-loop-shipcontract.md`](../proof/rl-loop-shipcontract.md) §Open carry-forwards for CF-1 through CF-7; dead-code-finder PASS; 5-verifier aspect panel PASS (disagreement <40% on the gate decision logic, which was the high-risk path); sprint-auditor PASS. |
