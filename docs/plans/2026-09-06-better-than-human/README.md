...
| 5 | Prospective memory wired (table exists, reads empty) | deferred intentions executed on cue | writer added: `insight_stream.py --prospective` (local GLM, keyword triggers ≥4 chars, lowercase); live keyword match case-folded (was case-sensitive → never fired on real texts). Gate = count of fired triggers in the daemon log over the next week |
