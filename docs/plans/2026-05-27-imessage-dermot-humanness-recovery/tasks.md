...
| **T9** | Add config parsing in `src/config/config_parse.c` to populate `agent.model_router.mlx_local_enabled` and `agent.model_router.mlx_local_model` from `~/.human/config.json`. Default `mlx_local_enabled = false` so the config edit is opt-in. | C2 | AC-5 | T8 | implementer | pending |
