...
**Goal:** Rewrite the SeaClaw chat engine and progressively enhance the entire UI to best-in-class quality — exceeding Apple, Anthropic Claude, and Steam in craft and polish.

**Architecture:** Nuclear rewrite of chat-view + sc-message-stream with a proper markdown rendering pipeline (marked + DOMPurify + Shiki + KaTeX). Progressive enhancement of all other surfaces with spring-based motion, glass depth, and feature completeness. Retrospective audit after each wave.
...
Uses `marked.lexer()` to tokenize markdown, then walks the AST to generate Lit `TemplateResult` nodes directly. Code blocks delegate to `<sc-code-block>`. Inline content uses `marked.parseInline()` + DOMPurify. Handles streaming partial content (unclosed code fences).

### Task 3: Create Code Block Component

**Files:**

- Create: `ui/src/components/sc-code-block.ts`
...
- Modify: `ui/src/components/sc-message-stream.ts`
...
Replace inline empty state (lines 796-813) with `<sc-empty-state>` + suggested prompt pills. Add `role="log"` and `aria-live="polite"` to message list. Remove hardcoded branch indicators. Clean up unused CSS (.md-blockquote, .md-table styles now handled by sc-message-stream).

### Task 6: Tests

Write tests for markdown.ts, sc-code-block, updated sc-message-stream, updated chat-view.
...
- Create `ui/src/components/sc-reasoning-block.ts` — collapsible chain-of-thought with expand/collapse animation

### Task 9: Progressive Streaming

- Enhance sc-message-stream for progressive character reveal during streaming
...
- Wire `--sc-spring-*` tokens to button presses, card hovers, modal entrances
...
- Real message branching (wire sc-message-branch to conversation data)
- Chat search (sc-chat-search component)
...
- sc-error-boundary component for graceful crash recovery
