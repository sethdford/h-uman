...
| 1.4 | Shared ctx lifetime + register semantics | `test_w7_*` + adversarial replace tests — **Done (2026-05-11).** Multi-cycle / audit-hook / safe-free invariants pinned by `test_w7_p14_multi_replace_cycle_deinits_each_evictee_once`, `test_w7_p14_reregister_same_ctx_no_double_deinit`, `test_w7_p14_audit_hook_survives_register_replace`, `test_w7_p14_records_free_before_replace_is_safe`. **Use-after-replace trap closed (2026-05-11):** `hu_memory_facade_read` now stashes the `(vt, ctx)` that produced each records array on a per-facade outstanding-reads list; `hu_memory_facade_records_free` routes by that captured origin and falls back to the slot only for untracked records. `hu_memory_facade_register_backend` returns `HU_ERR_MEMORY_BACKEND` if outstanding reads exist for the slot's current `(vt, ctx)`, forcing callers into the safe ordering (free first, replace second). Pinned by `test_w7_p14_register_refuses_replace_with_outstanding_reads` + `test_w7_p14_records_free_routes_to_origin_after_replace`. |
