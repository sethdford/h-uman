...
| `behavior/change.c` | **DECISION (2026-09-21, not wired):** `hu_behavior_change_input_t` needs 11 inputs; the turn has only chronotype. Fogg motivation/ability, consent, autonomy risk, burden and distress have no derivation anywhere; fabricating them would make the selector's safety gates vacuous in code that texts real people. Design the input derivation or delete by 2026-10-15 | `agent_turn.c:5052` |
...
| `hu_platform_realpath` | the mandated wrapper; 11 live sites bypassed it with raw `realpath()` (8 survived the deletions). **DONE (Task 20, 2026-09-21): all 8 sites routed through the wrapper; `scripts/check-*` guard fails on any new raw call** | `file_edit.c:127,159`, `process_util.c:24`, `update.c:126`, ... |
| `behavior/rel_dynamics.c` | **DECISION (2026-09-21, not done):** the 2026-05-29 "relocation" produced a redesigned API (`hu_rel_compute_state` / `hu_rel_build_prompt`, 12 funcs), not a copy of the `context/` API (`hu_rel_velocity_compute` / `hu_rel_dynamics_build_prompt`) that `daemon_reactive_prompt.c:1055-1068` uses live. "Finishing" it rewrites the live relationship prompt with no characterization test. Migrate the prompt path (behavior change) or delete the `behavior/` copy by 2026-10-15 | `include/human/daemon/context_facade.h:16` |
...
| `src/agent/max_tokens.c` | 242 | WIRED (HU_MAX_TOKENS_RESOLVE, shadow) |
| `src/agent/stop_sequence_registry.c` | 46 | WIRED (HU_STOP_SEQUENCES, shadow) |
| `src/agent/task_store.c` | 511 | WIRE |
| `src/agent/validators/cot_audit_validator.c` | 55 | DELETED |
| `src/behavior/change.c` | 163 | DECISION (inputs undefined) |
| `src/behavior/rel_dynamics.c` | 333 | DECISION (redesigned API, not a copy) |
