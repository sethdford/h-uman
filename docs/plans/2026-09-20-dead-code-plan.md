...
|---|---|---|
| `hu_imessage_find_inbound_unreplied` + `hu_follow_up_should_send_now` | the follow-up watcher is a literal stub; this is O2/O5 proactive reliability | `daemon_follow_up_watcher.c:73` |
...
| `agent/max_tokens.c` | still maintained; every provider falls back to a crude constant because `request->max_tokens` is never set | `agent_turn.c` / `agent_stream.c` request build |
| `behavior/change.c` | plan says verbatim "still not called from agent_turn.c" | `agent_turn.c:5052` |
| `validators/cot_audit_validator.c` | plan Task 11; chain machinery is live | `validators/default_chains.c:35-52` |
| `agent/stop_sequence_registry.c` | providers honor the field; nothing fills it | `agent_turn.c` request build |
