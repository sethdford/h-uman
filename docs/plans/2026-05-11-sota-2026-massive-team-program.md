...
> **2026-05-12 update — read this before trusting any row.**
> An evidence-based adversarial audit of the S2 ship claims (`docs/plans/2026-05-12-s2-gap-audit.md`, verdict `RESULT_critic=HAS_FINDINGS_0_2`) found four gaps between the rows below and the actual code state. Three pre-merge blockers (**HIGH-A** init-09 ingest provenance, **HIGH-B** init-02 daemon dispatcher wiring, **MED-A** init-11 Tier-1 channel typing) are landing as `fix/g0-*`, `fix/g1-*`, `fix/g3-*` PRs. Two initiatives (**LOW-A** init-04, **LOW-B** init-07) are honestly described as scaffolds in source comments but the status table previously omitted the SCAFFOLD label. **LOW-C** init-14 ships as adapter shims against synthetic smoke fixtures, not against live datasets — this is the source of the 7 pre-existing `test_eval_public_suites.c` failures that surface in any baseline build. The rows below have been re-labeled to PARTIAL / SCAFFOLD with explicit follow-up pointers so downstream agents and reviewers are not misled about what landed vs what shipped.
...
| 02 | molora-channels | code-architect | **S2 SHIPPED (PARTIAL) — dispatcher library + per-channel overlay routing + mlx_qwen3 STACK pool + HF5 typing-profile closure (commit `88d322b1`; 10279/10279 pass, +416 B ASan, +0 B release). Library was test-only; production daemon-side wiring shipping in `fix/g1-molora-daemon-wiring` (S2 gap audit HIGH-B).** | `2026-05-11-init-02-molora-channels.md` |
| 03 | apple-fm-provider | code-architect | **design done** | `2026-05-11-init-03-apple-fm-provider.md` |
| 04 | mlx-qwen3-provider | code-architect | **S1 SHIPPED (SCAFFOLD) — provider scaffold + S1.5(a) `load_adapter` widening + path-traversal guards** (commits in `96968daf` + `383cba08`). Production `chat()` path returns `HU_ERR_NOT_SUPPORTED`; JSON framer + spawn_helper land in P1/P2 (S2 gap audit LOW-A). | `2026-05-11-init-04-mlx-qwen3-provider.md` |
...
| 07 | thinkprm-verifier | code-architect | **S2 SHIPPED (SCAFFOLD) — `hu_verifier_panel_t` runtime + feature-hashing kernel + `human ml train-verifier` driver + opt-in agent_turn hook (default OFF byte-identical)** (commit `2092d3d4`; 10293/10293 pass, +37.3 KB ASan). Kernel is a deterministic synthetic checkpoint; real Qwen-class GPT forward + gradient training deferred to S3 pending init-04 (S2 gap audit LOW-B). | `2026-05-11-init-07-thinkprm-verifier.md` |
...
| 09 | memory-trust-tiers | security-reviewer | **S1 SHIPPED (PARTIAL) — write_trust library + MINJA guard + channel_trust + memory_poisoning suites green** (patched post-review). **Ingest-path provenance wiring (B3) was NOT shipped in S1**: `hu_personal_model_ingest` signature is unchanged and all four call sites pass `from_user=true` with no provenance, so MINJA gate is unreachable from production ingestion. Wiring ships in `fix/g0-ingest-provenance` (S2 gap audit HIGH-A). | `2026-05-11-init-09-memory-trust-tiers.md` |
...
| 11 | proactivity-typing | code-architect | **S1 SHIPPED (PARTIAL) — typing half + CLI channel wiring** (commits in `96968daf` + `383cba08`). Tier-1 channels (telegram/slack/discord/imessage) still bypass `hu_typing_send` via raw native calls; persona-driven WPM applies only to CLI. Tier-1 routing ships in `fix/g3-tier1-channel-typing` (S2 gap audit MED-A). | `2026-05-11-init-11-proactivity-typing.md` |
...
| 14 | public-benchmarks | code-architect | **S1 SHIPPED (SCAFFOLD) — 5 benchmark adapters + CLI + 19 regression tests** (commit `96968daf`). Adapters use `tests/fixtures/benchmarks/<name>/smoke.json` synthetic fixtures against a mock provider; no live LongMemEval / LoCoMo / EMPA / KnowU-Bench / ProAgentBench dataset is downloaded and no real benchmark API is called. 7 pre-existing failures in `test_eval_public_suites.c` belong to this scaffold tier (S2 gap audit LOW-C). | `2026-05-11-init-14-public-benchmarks.md` |
