...
static const facade_item_t FACADE_ITEMS[] = {
    {"Alice", HU_ENTITY_PERSON, "Acme", HU_ENTITY_ORGANIZATION, "works at since 2020",
     "where does alice work?", "Acme"},
    {"Bob", HU_ENTITY_PERSON, "Berlin", HU_ENTITY_PLACE, "moved to in 2019",
     "where does bob live now?", "Berlin"},
    {"Carla", HU_ENTITY_PERSON, "Yosemite", HU_ENTITY_PLACE, "climbed in last spring",
     "where did carla climb last spring?", "Yosemite"},
    {"Daniel", HU_ENTITY_PERSON, "Sourdough", HU_ENTITY_TOPIC, "learned during lockdown",
     "what did daniel learn during lockdown?", "Sourdough"},
    {"Eunji", HU_ENTITY_PERSON, "Vim", HU_ENTITY_TOPIC, "prefers as editor", "who prefers vim?",
     "Eunji"},
    {"Felix", HU_ENTITY_PERSON, "PineNuts", HU_ENTITY_TOPIC, "allergic to",
     "who is allergic to pine nuts?", "Felix"},
    {"Grace", HU_ENTITY_PERSON, "Marathon", HU_ENTITY_EVENT, "finished in three hours forty-two",
     "what was grace's marathon time?", "Marathon"},
    {"Hank", HU_ENTITY_PERSON, "Son", HU_ENTITY_PERSON, "born on october twelfth",
     "when was hank's son born?", "Son"},
    {"Iris", HU_ENTITY_PERSON, "Lisbon", HU_ENTITY_PLACE, "keeps beehive in",
     "where is iris keeping bees?", "Lisbon"},
    {"Jamal", HU_ENTITY_PERSON, "Accordion", HU_ENTITY_TOPIC, "taught by grandmother",
     "who taught jamal the accordion?", "Jamal"},
    {"Alice", HU_ENTITY_PERSON, "Bob", HU_ENTITY_PERSON, "collaborated on funding round",
     "when did alice and bob collaborate?", "Bob"},
    {"Alice", HU_ENTITY_PERSON, "Genmaicha", HU_ENTITY_TOPIC, "drinks every morning",
...
static hu_error_t upsert_entity_tracked(hu_graph_t *g, const char *cid, const char *name,
                                        hu_entity_type_t t, id_map_t *map, size_t *map_n,
                                        size_t map_cap) {
    ...
    hu_error_t err = hu_graph_upsert_entity(g, cid, strlen(cid), name, strlen(name), t, NULL, &id);
    if (err != HU_OK)
        return err;
    map[*map_n].name = name;
    map[(*map_n)++].id = id;
...
static size_t rank_of_entity(const hu_memory_record_t *recs, size_t n, int64_t target_id) {
...
static hu_error_t run_one_query(hu_memory_facade_t *m, hu_allocator_t *alloc, const char *cid,
                                const char *query, int64_t expect_entity_id, size_t *out_rank,
                                size_t *out_step_count) {
    hu_world_model_t *wm = NULL;
    hu_error_t err =
        hu_world_model_load(m, alloc, cid, strlen(cid), (int64_t)time(NULL) * 1000LL, &wm);
    if (err != HU_OK || !wm) {
        *out_rank = 0;
        *out_step_count = 0;
        return err;
...
static hu_error_t fr_run(void *ctx, hu_allocator_t *alloc, hu_evaluation_run_report_t *out) {
    (void)ctx;
    if (!alloc || !out)
    ...
    for (size_t i = 0; i < FACADE_N; i++) {
        ...
        err = upsert_entity_tracked(g, cid, it->subject, it->subj_type, name_to_id, &map_n, 32);
        if (err != HU_OK)
            goto fail;
        err = upsert_entity_tracked(g, cid, it->object, it->obj_type, name_to_id, &map_n, 32);
        if (err != HU_OK)
            goto fail;

        int64_t src_id = resolve_id(name_to_id, map_n, it->subject);
        int64_t tgt_id = resolve_id(name_to_id, map_n, it->object);
        err = hu_graph_upsert_relation(g, cid, strlen(cid), src_id, tgt_id, HU_REL_RELATED_TO, 1.0f,
                                       it->predicate, strlen(it->predicate));
        if (err != HU_OK)
            goto fail;
    }
    ...
    for (size_t i = 0; i < FACADE_N; i++) {
        ...
        hu_error_t qe =
            run_one_query(m, alloc, cid, FACADE_ITEMS[i].query, target_id, &rank, &step_count);
        if (qe != HU_OK)
            continue;
        scored++;
        ...
        if (rank > 0 && rank <= 10)
            hit_at_10++;
        if (trace && trace[0] == '1') {
            fprintf(stderr, "[facade-recall] q=%-50s expect=%-12s rank=%zu steps=%zu %s\n",
                    FACADE_ITEMS[i].query, FACADE_ITEMS[i].expect_name, rank, step_count,
                    rank == 1 ? "HIT@1" : (rank > 0 ? "MISS@1" : "MISS"));
        }
    }
    ...
    if (scored == 0) {
        (void)hu_evaluation_report_set_error(
            alloc, out, "facade-recall ran zero queries (id resolution failed)");
        out->finished_at_ms = now_ms();
        return HU_OK;
    }
