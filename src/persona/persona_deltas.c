...
static int recent_source_count(struct sqlite3 *db, const char *cid, int cid_len,
                               const char *source, int64_t now_ms) {
    sqlite3_stmt *st = NULL;
    const char *sql =
        "SELECT COUNT(*) FROM persona_deltas WHERE contact_id = ? AND source = ? "
        "AND proposed_at >= ?";
    if (sqlite3_prepare_v2(db, sql, -1, &st, NULL) != SQLITE_OK)
        return 0;
    sqlite3_bind_text(st, 1, cid, cid_len, SQLITE_STATIC);
...
hu_error_t hu_persona_evolver_run(hu_graph_t *graph, const char *contact_id,
    ...
    sqlite3_stmt *st = NULL;
    const char *sql =
        "SELECT id, kind, key, value, confidence, source FROM persona_deltas "
