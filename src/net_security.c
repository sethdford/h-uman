...
static int parse_ipv6(const char *s, unsigned short segs[8]) {
    ...
    if (dbl) {
        ...
        while (head < head_end && idx < 8) {
            ...
            while (head < head_end && *head != ':')
                head++;
            unsigned short v;
            if (parse_ipv6_quad(seg_start, head, &v) != 0)
                return -1;
            segs[idx++] = v;
        }
        /* Parse after :: */
        ...
        if (*tail) {
            ...
            while (*t && tail_count < 8) {
                ...
                while (*t && *t != ':')
                    t++;
                if (parse_ipv6_quad(ts, t, &tail_segs[tail_count]) != 0)
                    return -1;
                tail_count++;
            }
    ...
    while (*p && idx < 8) {
        ...
        while (*p && *p != ':')
            p++;
        if (parse_ipv6_quad(start, p, &segs[idx]) != 0)
            return -1;
        idx++;
    }
    return (idx == 8) ? 0 : -1;
...
static int is_non_global_v6(const unsigned short segs[8]) {
    ...
    if (segs[0] == 0 && segs[1] == 0 && segs[2] == 0 && segs[3] == 0 && segs[4] == 0 &&
        segs[5] == 0xFFFF) {
        unsigned char v4[4] = {(unsigned char)(segs[6] >> 8), (unsigned char)(segs[6] & 0xFF),
                               (unsigned char)(segs[7] >> 8), (unsigned char)(segs[7] & 0xFF)};
...
static int is_localhost_name(const char *host, size_t len) {
    if (len == 9 && ascii_ci_eq(host, "localhost", 9))
        return 1;
...
bool sc_validate_domain(const char *host, const char *const *allowed, size_t allowed_count) {
    ...
    for (size_t i = 0; i < allowed_count; i++) {
        ...
        if (plen >= 2 && pattern[0] == '*' && pattern[1] == '.') {
            ...
            size_t dlen = plen - 2;
            if (host_len >= dlen + 2 && memcmp(host + host_len - dlen, domain, dlen) == 0 &&
                host[host_len - dlen - 1] == '.')
                return true;
        }

        /* Implicit subdomain: api.example.com matches example.com */
        if (host_len > plen + 1 && host[host_len - plen - 1] == '.' &&
