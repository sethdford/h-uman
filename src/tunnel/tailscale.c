...
typedef struct hu_tailscale_tunnel {
    char *public_url;
    bool running;
    void *child_handle; /* FILE* from popen */
    hu_allocator_t *alloc;
} hu_tailscale_tunnel_t;

static void impl_stop(void *ctx);

static hu_tunnel_error_t impl_start(void *ctx, uint16_t local_port, char **public_url_out,
                                    size_t *url_len) {
    hu_tailscale_tunnel_t *self = (hu_tailscale_tunnel_t *)ctx;

    impl_stop(ctx);

#if HU_IS_TEST
    (void)local_port;
    char *mock = hu_strdup(self->alloc, "https://test-tailscale.ts.net");
    if (!mock)
...
static void impl_deinit(void *ctx, hu_allocator_t *alloc) {
    ...
    alloc->free(alloc->ctx, self, sizeof(hu_tailscale_tunnel_t));
}

static const hu_tunnel_vtable_t tailscale_vtable = {
    .start = impl_start,
    .stop = impl_stop,
    .deinit = impl_deinit,
...
hu_tunnel_t hu_tailscale_tunnel_create(hu_allocator_t *alloc) {
    hu_tailscale_tunnel_t *self =
        (hu_tailscale_tunnel_t *)alloc->alloc(alloc->ctx, sizeof(hu_tailscale_tunnel_t));
    if (!self)
