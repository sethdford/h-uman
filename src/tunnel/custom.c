...
#define HU_PORT_PLACEHOLDER "{port}"
#define HU_CMD_MAX          2048

typedef struct hu_custom_tunnel {
    char *public_url;
    bool running;
...
static hu_tunnel_error_t impl_start(void *ctx, uint16_t local_port, char **public_url_out,
                                    size_t *url_len) {
    hu_custom_tunnel_t *self = (hu_custom_tunnel_t *)ctx;

    impl_stop(ctx);

#if HU_IS_TEST
    (void)local_port;
    char *mock = hu_strdup(self->alloc, "https://custom-mock.example.net");
    if (!mock)
    ...
    *url_len = strlen(mock);
    ...
    if (!self->command_template || self->command_template_len == 0)
        return HU_TUNNEL_ERR_INVALID_COMMAND;

    /* Substitute {port} with actual port */
    char cmd[HU_CMD_MAX];
    const char *placeholder = strstr(self->command_template, HU_PORT_PLACEHOLDER);
    if (!placeholder) {
        ...
        size_t after = self->command_template_len - before - strlen(HU_PORT_PLACEHOLDER);
        ...
        int n = snprintf(cmd + before, sizeof(cmd) - before, "%u%s", (unsigned)local_port,
                         placeholder + strlen(HU_PORT_PLACEHOLDER));
    ...
    if (fgets(line, sizeof(line), f)) {
        ...
        while (len > 0 && (line[len - 1] == '\n' || line[len - 1] == '\r' || line[len - 1] == ' '))
            line[--len] = '\0';
        if (len > 0)
            found_url = hu_strndup(self->alloc, line, len);
    }
...
static void impl_deinit(void *ctx, hu_allocator_t *alloc) {
    ...
    alloc->free(alloc->ctx, self, sizeof(hu_custom_tunnel_t));
}

static const hu_tunnel_vtable_t custom_vtable = {
    .start = impl_start,
    .stop = impl_stop,
    .deinit = impl_deinit,
...
hu_tunnel_t hu_custom_tunnel_create(hu_allocator_t *alloc, const char *command_template,
                                    size_t command_template_len) {
    hu_custom_tunnel_t *self =
        (hu_custom_tunnel_t *)alloc->alloc(alloc->ctx, sizeof(hu_custom_tunnel_t));
    if (!self)
    ...
    if (command_template && command_template_len > 0) {
        self->command_template = hu_strndup(alloc, command_template, command_template_len);
        if (self->command_template)
