 ...
 * - Path security: hu_path_is_safe, hu_path_resolved_allowed
...
#define HU_FILE_EDIT_NAME "file_edit"
#define HU_FILE_EDIT_DESC "Find and replace text in a file"
#define HU_FILE_EDIT_PARAMS                                                                       \
    "{\"type\":\"object\",\"properties\":{\"path\":{\"type\":\"string\"},\"old_text\":{\"type\":" \
    "\"string\"},\"new_text\":{\"type\":\"string\"}},\"required\":[\"path\",\"old_text\",\"new_"  \
    "text\"]}"
#define HU_FILE_EDIT_MAX_SIZE (10 * 1024 * 1024) /* 10 MB */
#define HU_PATH_BUF           4096

typedef struct hu_file_edit_ctx {
    const char *workspace_dir;
    size_t workspace_dir_len;
    hu_security_policy_t *policy;
} hu_file_edit_ctx_t;

static hu_error_t file_edit_execute(void *ctx, hu_allocator_t *alloc, const hu_json_value_t *args,
                                    hu_tool_result_t *out) {
    hu_file_edit_ctx_t *c = (hu_file_edit_ctx_t *)ctx;
    if (!c || !args || !out) {
    ...
    const char *old_text = hu_json_get_string(args, "old_text");
    const char *new_text = hu_json_get_string(args, "new_text");

    if (!path || path[0] == '\0') {
        *out = hu_tool_result_fail("Missing 'path' parameter", 22);
        return HU_OK;
    }
    if (!old_text) {
        *out = hu_tool_result_fail("Missing 'old_text' parameter", 26);
        return HU_OK;
    }
    if (!new_text) {
        *out = hu_tool_result_fail("Missing 'new_text' parameter", 26);
        return HU_OK;
    }
    size_t old_len = strlen(old_text);
    if (old_len == 0) {
        *out = hu_tool_result_fail("old_text must not be empty", 26);
        return HU_OK;
    }

#if HU_IS_TEST
    /* In test mode, stub returns success to avoid filesystem side effects */
    char *msg = hu_strndup(alloc, "(file_edit stub in test)", 24);
    if (!msg) {
    ...
    *out = hu_tool_result_ok_owned(msg, 24);
    ...
    char full_path_buf[HU_PATH_BUF];
    const char *full_path = path;
    if (!is_absolute) {
        if (!hu_path_is_safe(path)) {
            *out = hu_tool_result_fail("Path not allowed: contains traversal or absolute path", 51);
            return HU_OK;
        }
        if (!c->workspace_dir || c->workspace_dir_len == 0) {
            *out = hu_tool_result_fail("Workspace not configured", 24);
            return HU_OK;
        }
        ...
        if (!c->policy || !c->policy->allowed_paths || c->policy->allowed_paths_count == 0) {
            *out =
                hu_tool_result_fail("Absolute paths not allowed (no allowed_paths configured)", 53);
            return HU_OK;
        }
    }
    ...
    char tmp_path_buf[HU_PATH_BUF + 16];
#ifndef _WIN32
    ...
    if (fd < 0) {
        ...
        if (err_msg)
            ...
            *out = hu_tool_result_fail("Failed to create temp file", 24);
        return HU_OK;
    }
    ...
    if (remove(resolved) != 0) { /* Delete original first on Windows */
        remove(tmp_path_buf);
        alloc->free(alloc->ctx, resolved, strlen(resolved) + 1);
        *out = hu_tool_result_fail("Failed to replace file", 21);
        return HU_OK;
    }
    if (rename(tmp_path_buf, resolved) != 0) {
        alloc->free(alloc->ctx, resolved, strlen(resolved) + 1);
    ...
    char *msg = hu_sprintf(alloc, "Replaced %zu bytes with %zu bytes in %s", (size_t)old_len,
                           new_len, path);
    if (!msg) {
        *out = hu_tool_result_fail("out of memory", 12);
        return HU_ERR_OUT_OF_MEMORY;
...
static const char *file_edit_name(void *ctx) {
    (void)ctx;
    return HU_FILE_EDIT_NAME;
}
static const char *file_edit_desc(void *ctx) {
    (void)ctx;
    return HU_FILE_EDIT_DESC;
}
static const char *file_edit_params(void *ctx) {
    (void)ctx;
    return HU_FILE_EDIT_PARAMS;
}

static void file_edit_deinit(void *ctx, hu_allocator_t *alloc) {
    if (!ctx)
        return;
    hu_file_edit_ctx_t *c = (hu_file_edit_ctx_t *)ctx;
...
static const hu_tool_vtable_t file_edit_vtable = {
...
hu_error_t hu_file_edit_create(hu_allocator_t *alloc, const char *workspace_dir,
                               size_t workspace_dir_len, hu_security_policy_t *policy,
                               hu_tool_t *out) {
    if (!alloc || !out)
        return HU_ERR_INVALID_ARGUMENT;
    hu_file_edit_ctx_t *c = (hu_file_edit_ctx_t *)alloc->alloc(alloc->ctx, sizeof(*c));
    if (!c)
        return HU_ERR_OUT_OF_MEMORY;
    memset(c, 0, sizeof(*c));
