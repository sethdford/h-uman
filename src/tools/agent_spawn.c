...
#define TOOL_DESC "Spawn a sub-agent to work on a task autonomously. Returns an agent ID for querying."
#define TOOL_PARAMS \
...
static sc_error_t agent_spawn_execute(void *ctx, sc_allocator_t *alloc,
    const sc_json_value_t *args, sc_tool_result_t *out)
{
    ...
    if (!task) { *out = sc_tool_result_fail("missing task", 12); return SC_OK; }

    const char *label = sc_json_get_string(args, "label");
    if (!label) label = "sub-agent";

#if SC_IS_TEST
    (void)c;
    char *msg = sc_sprintf(alloc, "{\"agent_id\":1,\"label\":\"%s\",\"status\":\"running\"}", label);
    *out = sc_tool_result_ok_owned(msg, msg ? strlen(msg) : 0);
    ...
    if (model) { cfg.model = model; cfg.model_len = strlen(model); }

    uint64_t agent_id = 0;
    sc_error_t err = sc_agent_pool_spawn(c->pool, &cfg, task, strlen(task), label, &agent_id);
    if (err != SC_OK) {
        *out = sc_tool_result_fail("spawn failed", 12); return SC_OK;
    }
...
static const char *agent_spawn_name(void *ctx) { (void)ctx; return TOOL_NAME; }
static const char *agent_spawn_desc(void *ctx) { (void)ctx; return TOOL_DESC; }
static const char *agent_spawn_params(void *ctx) { (void)ctx; return TOOL_PARAMS; }
static void agent_spawn_deinit(void *ctx, sc_allocator_t *alloc) {
    if (ctx) alloc->free(alloc->ctx, ctx, sizeof(agent_spawn_ctx_t));
}
