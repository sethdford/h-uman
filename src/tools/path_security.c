...
bool hu_path_is_safe(const char *path) {
    ...
    lower[len] = '\0';
    if (strstr(lower, "..%2f") || strstr(lower, "%2f..") || strstr(lower, "..%5c") ||
        strstr(lower, "%5c.."))
        return false;
...
static const char *const SYSTEM_BLOCKED[] = {
    "/System",      "/Library", "/bin",         "/sbin",        "/usr/bin", "/usr/sbin", "/usr/lib",
    "/usr/libexec", "/etc",     "/private/etc", "/private/var", "/dev",     "/boot",     "/proc",
    "/sys",         NULL};

#ifdef _WIN32
static const char *const SYSTEM_BLOCKED_WIN[] = {"C:\\Windows",
                                                 "C:\\Program Files",
                                                 "C:\\Program Files (x86)",
                                                 "C:\\ProgramData",
                                                 "C:\\System32",
...
bool hu_path_resolved_allowed(hu_allocator_t *alloc, const char *resolved, const char *ws_resolved,
    ...
    for (size_t i = 0; i < allowed_count; i++) {
        ...
        while (aplen > 0 && (ap[aplen - 1] == ' ' || ap[aplen - 1] == '\t'))
            aplen--;
        if (aplen == 0)
            continue;
        if (aplen == 1 && ap[0] == '*')
            return true;
        if (path_starts_with(resolved, ap))
            return true;
    }
