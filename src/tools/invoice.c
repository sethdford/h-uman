...
typedef struct { int placeholder; } invoice_ctx_t;
...
static sc_error_t invoice_execute(void *ctx, sc_allocator_t *alloc, const sc_json_value_t *args,
    ...
    if (strcmp(action, "create") == 0) {
        ...
        if (strcmp(format, "json") == 0) {
            ...
            n += snprintf(msg + n, buf_sz - (size_t)n,
                          ...
                          "|------|-----|-----------|-------|\n",
                          inv_num ? inv_num : "INV-001", date, from ? from : "",
                          to ? to : "", currency);
            ...
            n += snprintf(msg + n, buf_sz - (size_t)n,
                          "\n**Subtotal:** %.2f %s\n**Tax (%.0f%%):** %.2f %s\n"
                          "**Total:** %.2f %s\n",
                          subtotal, currency, tax_rate * 100, tax, currency,
    ...
    if (strcmp(action, "parse") == 0) {
        ...
        char *msg = sc_sprintf(alloc,
            ...
            "\"line_items\":2,\"source_length\":%zu}", strlen(data));
        *out = sc_tool_result_ok_owned(msg, msg ? strlen(msg) : 0);
#else
        char *msg = sc_sprintf(alloc, "{\"raw_length\":%zu,\"hint\":\"use AI to extract "
                               "structured fields from this text\"}", strlen(data));
        *out = sc_tool_result_ok_owned(msg, msg ? strlen(msg) : 0);
#endif
        return SC_OK;
    }
...
static const char *invoice_name(void *ctx) { (void)ctx; return TOOL_NAME; }
static const char *invoice_desc(void *ctx) { (void)ctx; return TOOL_DESC; }
static const char *invoice_params(void *ctx) { (void)ctx; return TOOL_PARAMS; }
static void invoice_deinit(void *ctx, sc_allocator_t *alloc) { (void)alloc; free(ctx); }

static const sc_tool_vtable_t invoice_vtable = {
    .execute = invoice_execute, .name = invoice_name, .description = invoice_desc,
    .parameters_json = invoice_params, .deinit = invoice_deinit,
};
