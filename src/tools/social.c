...
typedef struct { int placeholder; } social_ctx_t;
...
static sc_error_t social_execute(void *ctx, sc_allocator_t *alloc, const sc_json_value_t *args,
    ...
    if (strcmp(action, "post") == 0) {
        ...
        char *msg = sc_sprintf(alloc,
            "{\"posted\":true,\"platform\":\"%s\",\"id\":\"post_123\",\"content\":\"%s\"}",
    ...
    if (!token) {
        ...
        *out = sc_tool_result_fail(msg, msg ? strlen(msg) : 0);
        if (msg) alloc->free(alloc->ctx, msg, strlen(msg) + 1);
        return SC_OK;
    }
    ...
    if (strcmp(platform, "twitter") == 0 && strcmp(action, "post") == 0) {
        ...
        sc_error_t err = sc_http_post_json(alloc, "https://api.twitter.com/2/tweets", auth,
        ...
        *out = sc_tool_result_fail("LinkedIn posting requires OAuth2 + org ID setup", 47);
    } else {
...
static const char *social_name(void *ctx) { (void)ctx; return TOOL_NAME; }
static const char *social_desc(void *ctx) { (void)ctx; return TOOL_DESC; }
static const char *social_params(void *ctx) { (void)ctx; return TOOL_PARAMS; }
static void social_deinit(void *ctx, sc_allocator_t *alloc) { (void)alloc; free(ctx); }

static const sc_tool_vtable_t social_vtable = {
    .execute = social_execute, .name = social_name, .description = social_desc,
    .parameters_json = social_params, .deinit = social_deinit,
};
