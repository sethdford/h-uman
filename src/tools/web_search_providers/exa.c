...
hu_error_t hu_web_search_exa(hu_allocator_t *alloc, const char *query, size_t query_len, int count,
    ...
    hu_error_t err =
        hu_http_post_json_ex(alloc, EXA_API_URL, NULL, headers_buf, body_buf, (size_t)bn, &resp);
    if (err != HU_OK) {
        *out = hu_tool_result_fail("Exa search request failed", 26);
        return HU_OK;
    }
    if (resp.status_code < 200 || resp.status_code >= 300) {
        hu_http_response_free(alloc, &resp);
        *out = hu_tool_result_fail("Exa API error", 14);
        return HU_OK;
    }
