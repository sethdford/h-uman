...
hu_error_t hu_web_search_firecrawl(hu_allocator_t *alloc, const char *query, size_t query_len,
    ...
    hu_error_t err =
        hu_http_post_json(alloc, FIRECRAWL_API_URL, auth_buf, body_buf, (size_t)bn, &resp);
    if (err != HU_OK) {
        *out = hu_tool_result_fail("Firecrawl search request failed", 32);
        return HU_OK;
    }
    if (resp.status_code < 200 || resp.status_code >= 300) {
        hu_http_response_free(alloc, &resp);
        *out = hu_tool_result_fail("Firecrawl API error", 19);
        return HU_OK;
    }
    ...
    hu_json_value_t *results = hu_json_object_get(parsed, "data");
