...
#undef PHASE6_APPEND
                ...
                if (ctx_entries && ctx_count > 0 && !llm_decides) {
                    const char *aw_channel =
                        (ch->channel && ch->channel->vtable && ch->channel->vtable->name)
                            ? ch->channel->vtable->name(ch->channel->ctx)
                            : NULL;
                    size_t aw_channel_len = aw_channel ? strlen(aw_channel) : 0;
                    char *awareness_ctx = hu_conversation_build_awareness_on(
                        alloc, ctx_entries, ctx_count,
                        (agent && agent->persona) ? agent->persona : NULL, aw_channel,
                        aw_channel_len, &convo_ctx_len);
                ...
                /* Per-channel overlay hard cap.
                 *
                 * Personas can set `max_segment_chars` per channel (e.g. iMessage=160 for
                 * DM-style brevity, Slack=300 for slightly longer threaded replies). The
                 * ratio calibration above is generic; this enforces the user's explicit
                 * per-channel ceiling on top of it. Only tightens, never relaxes. */
                if (agent->persona && agent->active_channel && agent->active_channel_len > 0) {
                    const hu_persona_overlay_t *send_ov = hu_persona_find_overlay(
                        agent->persona, agent->active_channel, agent->active_channel_len);
                    if (send_ov && send_ov->max_segment_chars > 0 &&
                        (max_chars == 0 || send_ov->max_segment_chars < max_chars))
                        max_chars = send_ov->max_segment_chars;
                }
