...
#define HU_META_GRAPH_BASE     "https://graph.facebook.com/v21.0/"
#define HU_META_GRAPH_BASE_LEN (sizeof(HU_META_GRAPH_BASE) - 1)
#define HU_META_SIG_PREFIX     "sha256="
#define HU_META_SIG_PREFIX_LEN 7
#define HU_META_HMAC_HEX_LEN   64
...
bool hu_meta_verify_webhook(const char *body, size_t body_len, const char *signature,
    ...
    if (!body || !signature || !app_secret)
        return false;
    if (strncmp(signature, HU_META_SIG_PREFIX, HU_META_SIG_PREFIX_LEN) != 0)
        return false;
    const char *sig_hex = signature + HU_META_SIG_PREFIX_LEN;
    size_t sig_hex_len = strlen(sig_hex);
    if (sig_hex_len < HU_META_HMAC_HEX_LEN)
        return false;

    uint8_t computed[32];
    hu_hmac_sha256((const uint8_t *)app_secret, strlen(app_secret), (const uint8_t *)body, body_len,
                   computed);
    ...
    for (int i = 0; i < HU_META_HMAC_HEX_LEN; i++)
...
hu_error_t hu_meta_graph_send(hu_allocator_t *alloc, const char *access_token,
    ...
    char *url = NULL;
    size_t url_len = 0;
    hu_error_t err = hu_meta_graph_url(alloc, endpoint, endpoint_len, &url, &url_len);
    if (err != HU_OK)
        return err;
    ...
    err = hu_http_post_json(alloc, url, auth, json_body, json_body_len, &resp);
    alloc->free(alloc->ctx, auth, auth_len + 1);
    alloc->free(alloc->ctx, url, url_len + 1);
...
hu_error_t hu_meta_graph_url(hu_allocator_t *alloc, const char *path, size_t path_len,
    ...
    size_t total = HU_META_GRAPH_BASE_LEN + path_len + 1;
    char *url = (char *)alloc->alloc(alloc->ctx, total);
    if (!url)
        return HU_ERR_OUT_OF_MEMORY;

    memcpy(url, HU_META_GRAPH_BASE, HU_META_GRAPH_BASE_LEN);
    memcpy(url + HU_META_GRAPH_BASE_LEN, path, path_len);
    url[total - 1] = '\0';
