...
hu_error_t hu_imessage_poll(void *channel_ctx, hu_allocator_t *alloc, hu_channel_loop_msg_t *msgs,
    ...
    *out_count = 0;
    ...
    /* C6: detect date_retracted column (macOS Ventura+) once, cache forever.
     * chat.db schema doesn't change at runtime; probing every poll was
     * gratuitous overhead. -1 = unchecked, 0 = absent, 1 = present. */
    if (c->has_date_retracted_cached < 0) {
        sqlite3_stmt *col_check = NULL;
        if (sqlite3_prepare_v2(db, "SELECT date_retracted FROM message LIMIT 0", -1, &col_check,
                               NULL) == SQLITE_OK) {
            c->has_date_retracted_cached = 1;
            sqlite3_finalize(col_check);
        } else {
            c->has_date_retracted_cached = 0;
        }
    }
    bool has_date_retracted = (c->has_date_retracted_cached == 1);
