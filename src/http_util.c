...
sc_error_t sc_http_util_post(sc_allocator_t *alloc, const char *url, size_t url_len,
    ...
    sc_error_t err = sc_http_request(alloc, url, "POST", NULL, body, body_len, &resp);
    if (err != SC_OK)
        return err;
    *out_body = resp.body;
...
sc_error_t sc_http_util_get(sc_allocator_t *alloc, const char *url, size_t url_len,
