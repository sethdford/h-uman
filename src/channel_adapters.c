...
int hu_channel_adapters_parse_peer_kind(const char *raw, size_t len) {
    if (!raw)
        return -1;
    if (len >= 6 && strncmp(raw, "direct", 6) == 0)
        return (int)HU_CHAT_DIRECT;
    if (len >= 2 && strncmp(raw, "dm", 2) == 0)
        return (int)HU_CHAT_DIRECT;
    if (len >= 5 && strncmp(raw, "group", 5) == 0)
        return (int)HU_CHAT_GROUP;
    if (len >= 7 && strncmp(raw, "channel", 7) == 0)
        return (int)HU_CHAT_CHANNEL;
    return -1;
}

/* Polling descriptors for HU_LISTENER_POLLING channels (telegram, matrix, irc, signal, nostr). */
static const hu_polling_descriptor_t polling_table[] = {
    {"telegram", 1000}, {"discord", 2000}, {"matrix", 2000},   {"irc", 1000},
    {"signal", 2000},   {"nostr", 5000},   {"imessage", 3000}, {"email", 30000},
};
