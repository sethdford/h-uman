 ...
 */
#include "human/multimodal/calendar.h"
...
hu_error_t hu_calendar_add_event(hu_calendar_context_t *ctx, hu_allocator_t *alloc,
    ...
    if (ctx->event_count >= HU_CALENDAR_MAX_EVENTS)
        return HU_ERR_INVALID_ARGUMENT;

    hu_calendar_event_t *ev = &ctx->events[ctx->event_count];
    ev->title = hu_strndup(alloc, title, title_len);
    if (!ev->title)
...
static const char *event_type_label(hu_event_type_t type) {
    switch (type) {
    case HU_EVENT_MEETING:
        return "Meeting";
    case HU_EVENT_BIRTHDAY:
        return "Birthday";
    case HU_EVENT_REMINDER:
        return "Reminder";
    case HU_EVENT_DEADLINE:
        return "Deadline";
    case HU_EVENT_SOCIAL:
        return "Social";
    default:
        return "Event";
    }
...
#define HU_CALENDAR_BUF_INIT 512
#define HU_CALENDAR_BUF_GROW 256

static int append_event(char **buf, size_t *cap, size_t *len, const hu_calendar_event_t *ev,
                        int64_t now_ts, hu_allocator_t *alloc) {
    ...
    if (ev->title)
        need += ev->title_len;
    if (need > *cap) {
        size_t new_cap = *cap + HU_CALENDAR_BUF_GROW;
        while (new_cap < need)
            new_cap += HU_CALENDAR_BUF_GROW;
...
hu_error_t hu_calendar_build_context(const hu_calendar_context_t *ctx, hu_allocator_t *alloc,
    ...
    size_t cap = HU_CALENDAR_BUF_INIT;
    size_t len = 0;
    ...
    for (size_t i = 0; i < ctx->event_count; i++) {
        const hu_calendar_event_t *ev = &ctx->events[i];
    ...
    for (size_t i = 0; i < ctx->event_count; i++) {
        const hu_calendar_event_t *ev = &ctx->events[i];
        if (ev->type != HU_EVENT_BIRTHDAY || ev->start_ts < now_ts || ev->start_ts > week_end)
            continue;
        if (append_event(&buf, &cap, &len, ev, now_ts, alloc) != 0) {
            alloc->free(alloc->ctx, buf, cap);
            return HU_ERR_OUT_OF_MEMORY;
...
void hu_calendar_deinit(hu_calendar_context_t *ctx, hu_allocator_t *alloc) {
    if (!ctx || !alloc)
        return;
    for (size_t i = 0; i < ctx->event_count; i++) {
        hu_calendar_event_t *ev = &ctx->events[i];
        if (ev->title) {
            hu_str_free(alloc, ev->title);
            ev->title = NULL;
        }
        ev->title_len = 0;
        if (ev->location) {
            hu_str_free(alloc, ev->location);
            ev->location = NULL;
        }
        ev->location_len = 0;
        if (ev->contact) {
            hu_str_free(alloc, ev->contact);
            ev->contact = NULL;
        }
        ev->contact_len = 0;
    }
