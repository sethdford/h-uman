...
static sc_session_entry_t **find_bucket(sc_session_manager_t *mgr, const char *key) {
    uint32_t h = hash_key(key) % SC_SESSION_MAP_CAP;
    return &mgr->buckets[h];
}

static sc_session_t *find_session(sc_session_manager_t *mgr, const char *key) {
...
static void free_session_messages(sc_session_t *s, sc_allocator_t *alloc) {
    ...
    if (s->messages) {
        alloc->free(alloc->ctx, s->messages, s->message_cap * sizeof(sc_session_message_t));
    }
    s->messages = NULL;
    s->message_count = 0;
    s->message_cap = 0;
...
sc_session_t *sc_session_get_or_create(sc_session_manager_t *mgr, const char *session_key) {
    ...
    sc_session_t *new_s = mgr->alloc->alloc(mgr->alloc->ctx, sizeof(sc_session_t));
    if (!new_s)
        return NULL;
    memset(new_s, 0, sizeof(*new_s));
    size_t klen = strlen(session_key);
    if (klen >= SC_SESSION_KEY_LEN)
        klen = SC_SESSION_KEY_LEN - 1;
    ...
    sc_session_entry_t *ent = mgr->alloc->alloc(mgr->alloc->ctx, sizeof(sc_session_entry_t));
    if (!ent) {
...
sc_error_t sc_session_append_message(sc_session_t *s, sc_allocator_t *alloc, const char *role,
                                     const char *content) {
    if (!s || !alloc || !role || !content)
        return SC_ERR_INVALID_ARGUMENT;
    if (s->message_count >= s->message_cap) {
        size_t new_cap = s->message_cap ? s->message_cap * 2 : 8;
        if (new_cap > SC_SESSION_MSG_CAP)
            new_cap = SC_SESSION_MSG_CAP;
        sc_session_message_t *n =
            alloc->realloc(alloc->ctx, s->messages, s->message_cap * sizeof(sc_session_message_t),
    ...
    if (rlen >= SC_SESSION_MSG_ROLE_LEN)
        rlen = SC_SESSION_MSG_ROLE_LEN - 1;
    memcpy(m->role, role, rlen);
    m->role[rlen] = '\0';
    size_t clen = strlen(content) + 1;
    if (clen > SC_SESSION_MSG_CONTENT_CAP)
        clen = SC_SESSION_MSG_CONTENT_CAP;
...
sc_session_summary_t *sc_session_list(sc_session_manager_t *mgr, sc_allocator_t *alloc,
                                      size_t *out_count) {
    ...
    *out_count = 0;
    ...
    if (n == 0)
        return NULL;
    sc_session_summary_t *arr = alloc->alloc(alloc->ctx, n * sizeof(sc_session_summary_t));
    if (!arr)
        return NULL;
    size_t idx = 0;
...
sc_error_t sc_session_patch(sc_session_manager_t *mgr, const char *session_key, const char *label) {
    if (!mgr || !session_key)
        return SC_ERR_INVALID_ARGUMENT;
    if (!label)
        return SC_ERR_INVALID_ARGUMENT;
...
sc_error_t sc_session_load(sc_session_manager_t *mgr, const char *path) {
    ...
    for (size_t i = 0; i < root->data.array.len; i++) {
        ...
        if (label) {
            ...
            if (llen >= SC_SESSION_LABEL_LEN)
                llen = SC_SESSION_LABEL_LEN - 1;
