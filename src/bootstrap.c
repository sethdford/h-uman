    ...
    if (with_channels) {
        ...
        if (cfg->channels.imessage.default_target && ch_count < HU_BOOTSTRAP_CHANNELS_MAX) {
            ...
            if (err == HU_OK) {
                ...
                /* US-43.3: operator opt-out flag flows from config into the
                 * channel state. Default is true; explicit false disables
                 * the courtesy reply path system-wide. */
                hu_imessage_set_courtesy_replies_enabled(
                    &bi->channel_slots[ch_count], cfg->channels.imessage.courtesy_replies_enabled);
                bi->channels[ch_count].channel_ctx = bi->channel_slots[ch_count].ctx;
                bi->channels[ch_count].channel = &bi->channel_slots[ch_count];
