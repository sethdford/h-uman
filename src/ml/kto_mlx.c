...
static hu_error_t kto_write_jsonl(const hu_preference_pair_t *pairs, size_t n, char *out_path,
                                  size_t out_path_cap) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (is_two_sided)
            continue;
        if (!is_desirable && !is_undesirable)
            continue;
...
static hu_error_t kto_mlx_step(void *vctx, hu_allocator_t *alloc, const hu_preference_pair_t *pairs,
    ...
    hu_error_t serr =
        hu_ml_resolve_script_path("kto_mlx_train.py", script_path, sizeof(script_path));
    if (serr != HU_OK) {
        unlink(jsonl_path);
        return serr;
    }
 ...
 * the same standardization (Phase 2 audit b6a71f81). */
    ...
    /* Check the snprintf return value — same pattern as dpo_real_mlx.c:251.
     * Both buffers are 512 bytes and the literal suffix "/adapters.safetensors"
     * adds 21, so Linux GCC -Werror=format-truncation flags this as a
     * potential 533-byte-into-512-byte write. Treating truncation as
     * HU_ERR_PROVIDER_RESPONSE is semantically equivalent to the previous
     * behavior (a truncated path would then fail stat() below) and
     * silences the warning honestly. */
...
static const char *kto_mlx_name(void *vctx) {
    (void)vctx;
    return "kto_mlx";
}
...
hu_error_t hu_kto_mlx_create(hu_allocator_t *alloc, const hu_rl_trainer_config_t *config,
                             hu_rl_trainer_t *out) {
    if (!alloc || !config || !out)
        return HU_ERR_INVALID_ARGUMENT;
