...
static hu_error_t alloc_signals(hu_allocator_t *alloc, size_t count, hu_training_signal_t **out) {
    if (count == 0) {
        *out = NULL;
        return HU_OK;
    }
    hu_training_signal_t *s = (hu_training_signal_t *)alloc->alloc(alloc->ctx, count * sizeof(*s));
    if (!s)
        return HU_ERR_OUT_OF_MEMORY;
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (as_dpo_dispreferred) {
            ...
            snprintf(signals[i].as.dpo.prompt, sizeof(signals[i].as.dpo.prompt),
                     "[verifier-flag kind=%d key=%s]", (int)deltas[i].kind, deltas[i].key);
            /* preferred: explicit "abstain" — the response the verifier
             * would have allowed. dispreferred: the rejected delta value. */
            snprintf(signals[i].as.dpo.preferred, sizeof(signals[i].as.dpo.preferred), "[abstain]");
            snprintf(signals[i].as.dpo.dispreferred, sizeof(signals[i].as.dpo.dispreferred), "%s",
                     deltas[i].value);
...
hu_error_t hu_learner_signals_from_persona_deltas(hu_memory_facade_t *m, hu_allocator_t *alloc,
                                                  const char *contact_id, size_t cid_len,
                                                  hu_training_signal_t **out, size_t *out_count) {
    return signals_from_deltas_with_status(m, alloc, contact_id, cid_len, HU_DELTA_STATUS_APPLIED,
