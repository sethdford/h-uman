...
static hu_error_t grpo_mlx_step(void *vctx, hu_allocator_t *alloc,
     ...
     *
     * Check the snprintf return value — both buffers are 512 bytes and
     * "/adapter_model.safetensors" adds 26, so Linux GCC
     * -Werror=format-truncation flags this as a potential 538-byte-into-
     * 512-byte write. Same return-value pattern as kto_mlx.c:258 and
     * dpo_real_mlx.c:251 (CI rot cleanup 2026-05-17). */
    int path_len = snprintf(out->adapter_path, sizeof(out->adapter_path),
