...
/* MLX backend probes are only used inside __APPLE__-guarded call sites
 * (hu_rl_trainer_create_{dpo,kto,grpo}). On Linux they were defined but
 * never called, tripping -Werror=unused-function under the dev-strict
 * CFLAGS. Wrapping definitions in the same guard keeps the binding
 * one-sided and silences the warning honestly. */
 ...
 * deferring failure to popen() time. Guarded on __APPLE__ — Linux
 * never calls this; see the matching guard on mlx_dpo_available above. */
 ...
 *
 * Guarded on __APPLE__ — Linux never calls this. */
