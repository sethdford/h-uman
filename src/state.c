...
sc_process_state_t sc_state_get_process(const sc_state_manager_t *mgr) {
    if (!mgr)
        return SC_PROCESS_STATE_STOPPED;
    ensure_mutex();
...
void sc_state_set_last_channel(sc_state_manager_t *mgr, const char *channel, const char *chat_id) {
    ...
    if (channel) {
        ...
        if (n >= SC_STATE_CHANNEL_LEN)
            n = SC_STATE_CHANNEL_LEN - 1;
    ...
    if (chat_id) {
        ...
        if (n >= SC_STATE_CHAT_ID_LEN)
            n = SC_STATE_CHAT_ID_LEN - 1;
