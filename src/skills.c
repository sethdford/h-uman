...
sc_error_t sc_skills_list(sc_allocator_t *alloc, const char *workspace_dir,
    ...
    if (!alloc || !out_skills || !out_count) return SC_ERR_INVALID_ARGUMENT;
    *out_skills = NULL;
    *out_count = 0;
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        copy[i].description = from[i].description ? sc_strdup(alloc, from[i].description) : sc_strdup(alloc, "");
...
void sc_skills_free(sc_allocator_t *alloc, sc_skill_t *skills, size_t count) {
    if (!alloc || !skills) return;
    for (size_t i = 0; i < count; i++) {
        if (skills[i].name) alloc->free(alloc->ctx, skills[i].name, strlen(skills[i].name) + 1);
        if (skills[i].description) alloc->free(alloc->ctx, skills[i].description, strlen(skills[i].description) + 1);
        if (skills[i].parameters) alloc->free(alloc->ctx, skills[i].parameters, strlen(skills[i].parameters) + 1);
    }
