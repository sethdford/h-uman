...
static const char *impl_name(void *ctx) {
    hu_rpi_ctx_t *s = (hu_rpi_ctx_t *)ctx;
...
static void impl_destroy(void *ctx, hu_allocator_t *alloc) {
    hu_rpi_ctx_t *s = (hu_rpi_ctx_t *)ctx;
    alloc->free(alloc->ctx, s, sizeof(hu_rpi_ctx_t));
}
...
static const hu_peripheral_vtable_t rpi_vtable = {
    .name = impl_name,
...
hu_peripheral_t hu_rpi_peripheral_create(hu_allocator_t *alloc) {
    ...
    hu_rpi_ctx_t *s = (hu_rpi_ctx_t *)alloc->alloc(alloc->ctx, sizeof(hu_rpi_ctx_t));
    if (!s)
        return (hu_peripheral_t){.ctx = NULL, .vtable = NULL};
    ...
    return (hu_peripheral_t){.ctx = s, .vtable = &rpi_vtable};
}
