...
sc_error_t sc_tom_init(sc_belief_state_t *state, sc_allocator_t *alloc, const char *contact_id,
    ...
    char *dup = sc_strndup(alloc, contact_id, len);
    if (!dup)
...
static sc_belief_t *find_belief_by_topic(sc_belief_state_t *state, const char *topic,
                                         size_t topic_len) {
    for (size_t i = 0; i < state->belief_count; i++) {
        sc_belief_t *b = &state->beliefs[i];
...
sc_error_t sc_tom_record_belief(sc_belief_state_t *state, sc_allocator_t *alloc, const char *topic,
    ...
    sc_belief_t *existing = find_belief_by_topic(state, topic, len);
    if (existing) {
        existing->type = type;
    ...
    if (state->belief_count >= SC_TOM_MAX_BELIEFS)
        return SC_ERR_OUT_OF_MEMORY;

    char *dup = sc_strndup(alloc, topic, len);
    if (!dup)
        return SC_ERR_OUT_OF_MEMORY;

    sc_belief_t *b = &state->beliefs[state->belief_count];
    b->topic = dup;
...
static const char *belief_type_label(sc_belief_type_t type) {
    switch (type) {
    case SC_BELIEF_KNOWS:
        return "KNOWS";
    case SC_BELIEF_ASSUMES:
        return "ASSUMES";
    case SC_BELIEF_UNAWARE:
        return "UNAWARE";
    case SC_BELIEF_MISTAKEN:
        return "MISTAKEN";
    default:
        return "UNKNOWN";
    }
...
static bool belief_shows_confidence(sc_belief_type_t type) {
    return type == SC_BELIEF_KNOWS || type == SC_BELIEF_ASSUMES || type == SC_BELIEF_MISTAKEN;
}
...
sc_error_t sc_tom_build_context(const sc_belief_state_t *state, sc_allocator_t *alloc, char **out,
    ...
    for (int t = SC_BELIEF_KNOWS; t <= SC_BELIEF_MISTAKEN; t++) {
        sc_belief_type_t type = (sc_belief_type_t)t;
        bool first = true;
        for (size_t i = 0; i < state->belief_count; i++) {
...
void sc_tom_deinit(sc_belief_state_t *state, sc_allocator_t *alloc) {
    if (!state || !alloc)
        return;
    sc_str_free(alloc, state->contact_id);
    state->contact_id = NULL;
    state->contact_id_len = 0;
    for (size_t i = 0; i < state->belief_count; i++) {
        sc_str_free(alloc, state->beliefs[i].topic);
