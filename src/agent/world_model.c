...
static hu_error_t ensure_negative_memory_schema(struct sqlite3 *db) {
    ...
    for (size_t i = 0; stmts[i]; i++) {
        if (run_ddl_(db, stmts[i]) != SQLITE_OK)
            return HU_ERR_IO;
    }
...
static hu_error_t negative_memory_list_sqlite(struct sqlite3 *db, hu_allocator_t *alloc,
    ...
    *out_count = 0;
    ...
    sqlite3_stmt *st = NULL;
    const char *sql = "SELECT id, text, scope, reason, confidence_mean, confidence_variance, "
...
static hu_error_t negative_memory_add_sqlite(struct sqlite3 *db, const char *contact_id,
    ...
    sqlite3_stmt *st = NULL;
    const char *sql = "INSERT INTO negative_memory(contact_id, text, scope, reason, "
...
hu_error_t hu_negative_memory_add_facade_gated(hu_memory_facade_t *m, const char *contact_id,
    ...
    if (outcome == HU_WRITE_OUTCOME_QUARANTINE) {
        if (to_insert.belief.mean > 0.5f)
            to_insert.belief.mean = 0.5f;
        if (to_insert.belief.variance < 0.25f)
            to_insert.belief.variance = 0.25f;
    }
...
static hu_error_t copy_entities(hu_allocator_t *alloc, const hu_memory_entity_row_t *src, size_t n,
                                hu_memory_entity_row_t **out) {
    if (n == 0) {
        *out = NULL;
        return HU_OK;
    }
        ...
        if (src[i].name && src[i].name_len > 0) {
            ...
            if (!dup) {
                ...
                for (size_t j = 0; j < i; j++)
                    xfree(alloc, arr[j].name, arr[j].name_len + 1);
...
static void free_entities_local(hu_allocator_t *alloc, hu_memory_entity_row_t *arr, size_t n) {
    if (!arr)
        return;
    for (size_t i = 0; i < n; i++)
        xfree(alloc, arr[i].name, arr[i].name_len + 1);
    xfree(alloc, arr, n * sizeof(*arr));
}
...
hu_error_t hu_world_model_build(hu_memory_facade_t *m, hu_allocator_t *alloc,
    ...
    *out = NULL;
    ...
    size_t copy_len = cid_len < sizeof(wm->contact_id) - 1 ? cid_len : sizeof(wm->contact_id) - 1;
     ...
     * as a distinct facade kind. */
    {
        ...
        if (wm_limit_env && *wm_limit_env) {
            ...
            if (v >= 16 && v <= 8192)
                wm_entity_limit = (size_t)v;
        }
        if (hu_memory_facade_list_entities(m, alloc, contact_id, cid_len, wm_entity_limit, &ents,
                                           &n) == HU_OK &&
            n > 0) {
                ...
                for (size_t i = 0; i < rn && cn < change_cap; i++) {
                    ...
                    if (kind == HU_WORLD_CHANGE_SUPERSEDED) {
                        snprintf(changes[cn].summary, sizeof(changes[cn].summary),
                                 "rel #%lld supersedes #%lld", (long long)r->id,
                                 (long long)r->supersedes_id);
                    } else {
                        snprintf(changes[cn].summary, sizeof(changes[cn].summary),
                                 "rel #%lld retracted at %lld", (long long)r->id,
        ...
        if (wm->entities && wm->entities_count > 0) {
            ...
            if (stage_cap < cap_target)
                stage_cap = cap_target;
...
#ifdef HU_ENABLE_SQLITE
    {
        ...
        if (db) {
            ...
            if (hu_emotional_residue_get_active(alloc, db, contact_id, cid_len, now_ts, &residues,
                                                &residue_n) == HU_OK &&
                residue_n > 0) {
                ...
                for (size_t i = 0; i < residue_n; i++) {
                    ...
                    if (age_ts <= 60 * 60 && residues[i].valence < -0.3 &&
                        residues[i].intensity > 0.6) {
                        anger_count++;
                    }
                    if (residues[i].valence < -0.15) {
                        if (earliest_neg_ts == 0 || residues[i].created_at < earliest_neg_ts) {
                ...
                if (anger_term > 1.0f)
                    anger_term = 1.0f;
                float complaint_term = (float)wm->pressure.sustained_complaint_minutes / 30.0f;
                if (complaint_term > 1.0f)
                    complaint_term = 1.0f;
...
void hu_world_model_merge_personal(hu_world_model_t *wm, const hu_personal_model_t *pm) {
    if (!wm || !pm)
        return;
    if (!hu_personal_model_has_content(pm))
        return;
...
static const char *overlay_cannot_clause_(const char *axis, const char *value) {
    ...
    if (strcasecmp(axis, "face_saving") == 0) {
        if (strcasecmp(value, "high") == 0)
            return "challenge them in front of others";
    } else if (strcasecmp(axis, "disagreement_style") == 0) {
        if (strcasecmp(value, "indirect") == 0)
            return "disagree bluntly";
        if (strcasecmp(value, "avoidant") == 0)
            return "force the disagreement to a head";
    } else if (strcasecmp(axis, "silence_tolerance") == 0) {
        if (strcasecmp(value, "low") == 0)
            return "leave long silences without checking in";
    } else if (strcasecmp(axis, "vulnerability_tier") == 0) {
        if (strcasecmp(value, "low") == 0)
            return "open with heavy emotional content";
        if (strcasecmp(value, "none") == 0)
            return "share vulnerable material at all";
    }
    return NULL;
}
...
static const char *overlay_can_clause_(const char *axis, const char *value) {
    ...
    if (strcasecmp(axis, "directness") == 0) {
        if (strcasecmp(value, "direct") == 0)
            return "give direct, unhedged answers";
        if (strcasecmp(value, "blunt") == 0)
            return "be blunt without softening";
    } else if (strcasecmp(axis, "vulnerability_tier") == 0) {
        if (strcasecmp(value, "high") == 0)
            return "engage with vulnerable material";
        if (strcasecmp(value, "deep") == 0)
            return "hold space for deep emotional content";
    } else if (strcasecmp(axis, "disagreement_style") == 0) {
        if (strcasecmp(value, "direct") == 0)
            return "push back when I see it differently";
    }
    return NULL;
}
...
static void self_model_append_drift_history_(hu_world_model_t *wm, const char *kind_str) {
    ...
    for (size_t i = 1; i < HU_SELF_DRIFT_HISTORY; i++) {
        memcpy(wm->self_model.recent_drift_history[i - 1], wm->self_model.recent_drift_history[i],
...
void hu_world_model_merge_persona(hu_world_model_t *wm, const struct hu_persona *persona,
    ...
    if (persona->name && persona->name[0])
        identity_completeness = 0.5f;
    if (persona->identity && persona->identity[0])
        identity_completeness = 0.85f;
    ...
    if (ov) {
        ...
        if (cn1)
            tom_append_str_(wm->tom.user_expects_we_cannot, sizeof(wm->tom.user_expects_we_cannot),
                            cn1);
        if (cn2)
            tom_append_str_(wm->tom.user_expects_we_cannot, sizeof(wm->tom.user_expects_we_cannot),
                            cn2);
        ...
        if (cp1)
            tom_append_str_(wm->tom.user_expects_we_can, sizeof(wm->tom.user_expects_we_can), cp1);
        if (cp2)
            tom_append_str_(wm->tom.user_expects_we_can, sizeof(wm->tom.user_expects_we_can), cp2);
        if (cp3)
            tom_append_str_(wm->tom.user_expects_we_can, sizeof(wm->tom.user_expects_we_can), cp3);
        if (cp1 || cp2 || cp3)
            new_signals++;
    }
    ...
    if (wm->contact_id[0]) {
        for (size_t i = 0; i < persona->contacts_count; i++) {
            ...
            /* 2026-05-16 P3-8: strict contact_id equality only.  The previous
             * fallback to strcasecmp(c->name, wm->contact_id) routed the wrong
             * contact's persona profile into the system prompt when a channel
             * handle (e.g. Slack "betty") happened to equal another contact's
             * display name ("Betty").  contact_id is the only collision-free
             * lookup key. */
            if (!c->contact_id || !c->contact_id[0])
                continue;
            if (strcmp(c->contact_id, wm->contact_id) != 0)
                continue;
            size_t added = 0;
            for (size_t j = 0; j < c->allowed_behaviors_count && added < 3; j++) {
                ...
                if (!b || !b[0])
                    continue;
                tom_append_str_(wm->tom.user_expects_we_can, sizeof(wm->tom.user_expects_we_can),
                                b);
                added++;
            }
            if (added > 0)
    ...
    }
    {
        ...
        for (size_t i = 0; i < persona->situational_directions_count && added < 2; i++) {
            ...
            if (!d->instruction || !d->instruction[0])
                continue;
            tom_append_str_(wm->tom.user_expects_we_can, sizeof(wm->tom.user_expects_we_can),
                            d->instruction);
            added++;
    ...
    for (size_t i = 0; i < deltas_count; i++) {
        ...
        if (d->status != HU_DELTA_STATUS_APPLIED)
            continue;
        ...
        if (hist_kind)
            self_model_append_drift_history_(wm, hist_kind);
        switch (d->kind) {
        case HU_PERSONA_DELTA_BOUNDARY:
        case HU_PERSONA_DELTA_VOCAB_AVOID:
            tom_append_str_(wm->tom.user_expects_we_cannot, sizeof(wm->tom.user_expects_we_cannot),
        ...
        case HU_PERSONA_DELTA_TONE:
        case HU_PERSONA_DELTA_LENGTH:
            tom_append_str_(wm->tom.interaction_style, sizeof(wm->tom.interaction_style), d->value);
            new_signals++;
            break;
        default:
            break;
    ...
    if (wm->tom.confidence_history_count < HU_TOM_CONFIDENCE_HISTORY) {
        wm->tom.confidence_history[wm->tom.confidence_history_count] = wm->tom.confidence.mean;
        ...
        wm->tom.confidence_history[HU_TOM_CONFIDENCE_HISTORY - 1] = wm->tom.confidence.mean;
    }
}
...
static bool goal_anchor_matches_(const char *goal, size_t goal_len, const char *name,
                                 size_t name_len) {
    if (!goal || goal_len == 0 || !name || name_len == 0)
        return false;
...
hu_error_t hu_world_model_rerank_for_goal(hu_world_model_t *wm, const char *goal_text,
    ...
    if (wm->entities && wm->entities_count > 1) {
        ...
        for (size_t r = 0; r < wm->entities_count; r++) {
            if (goal_anchor_matches_(goal_text, goal_text_len, wm->entities[r].name,
                                     wm->entities[r].name_len)) {
                if (r != w) {
    ...
    if (wm->relations && wm->relations_count > 1) {
        ...
        for (size_t r = 0; r < wm->relations_count; r++) {
            ...
            bool match =
                goal_anchor_matches_(goal_text, goal_text_len, rel->source_name,
                                     rel->source_name_len) ||
                goal_anchor_matches_(goal_text, goal_text_len, rel->target_name,
                                     rel->target_name_len) ||
                goal_anchor_matches_(goal_text, goal_text_len, rel->context, rel->context_len);
            if (match) {
                if (r != w) {
            ...
            if (goal_anchor_matches_(goal_text, goal_text_len, t, strlen(t))) {
                if (r != w) {
                    ...
                    for (size_t k = r; k > w + 1; k--) {
                        ...
                        memcpy(wm->recent_topics[k], wm->recent_topics[k - 1], sizeof(t2));
...
void hu_world_model_merge_self_capabilities(hu_world_model_t *wm, const struct hu_tool *tools,
    ...
    size_t cap =
        sizeof(wm->self_model.capabilities) / sizeof(wm->self_model.capabilities[0]); /* 6 today */
...
void hu_world_model_merge_self_recent_tools(hu_world_model_t *wm, const char *const *tool_names,
    ...
    if (!tool_names || tool_names_count == 0)
        return;

    size_t cap =
        sizeof(wm->self_model.recent_tools_used) / sizeof(wm->self_model.recent_tools_used[0]);
    size_t name_cap = sizeof(wm->self_model.recent_tools_used[0]);

    for (size_t i = 0; i < tool_names_count && wm->self_model.recent_tools_used_count < cap; i++) {
        ...
        if (nlen >= name_cap)
            nlen = name_cap - 1;
        memcpy(wm->self_model.recent_tools_used[wm->self_model.recent_tools_used_count], nm, nlen);
...
void hu_world_model_free(hu_allocator_t *alloc, hu_world_model_t *wm) {
    ...
    if (wm->relations) {
        for (size_t i = 0; i < wm->relations_count; i++) {
            if (wm->relations[i].provenance) {
                xfree(alloc, wm->relations[i].provenance, wm->relations[i].provenance_len + 1);
            }
        }
    ...
    if (wm->recent_changes) {
        xfree(alloc, wm->recent_changes, wm->recent_changes_count * sizeof(*wm->recent_changes));
    }
    ...
    if (wm->hyperedges) {
        for (size_t i = 0; i < wm->hyperedges_count; i++) {
            if (wm->hyperedges[i].members) {
                xfree(alloc, wm->hyperedges[i].members,
                      wm->hyperedges[i].members_count * sizeof(*wm->hyperedges[i].members));
            }
        ...
        xfree(alloc, wm->hyperedges, wm->hyperedges_count * sizeof(*wm->hyperedges));
    }
    if (wm->negatives)
        hu_negative_memory_free(alloc, wm->negatives, wm->negatives_count);
...
static struct wm_cache_entry *cache_lookup_locked_(const char *contact_id, size_t cid_len,
    ...
    for (size_t i = 0; i < s_cache_slots; i++) {
        ...
        if (strncmp(e->contact_id, contact_id, cid_len) != 0)
            continue;
        if (e->contact_id[cid_len] != '\0')
            continue;
        /* Channel must match exactly (including the empty-string default). */
        if (e->channel_len != channel_len)
            continue;
        if (channel_len > 0 && memcmp(e->channel, channel, channel_len) != 0)
            continue;
        return e;
    }
    return NULL;
...
static struct wm_cache_entry *cache_evict_slot_locked_(void) {
    ...
    for (size_t i = 0; i < s_cache_slots; i++) {
        ...
        if (!e->wm)
            return e;
        if (e->last_access < oldest->last_access)
            oldest = e;
    }
    if (oldest->wm) {
...
static hu_world_model_t *clone_wm(hu_allocator_t *alloc, const hu_world_model_t *src) {
    ...
    *wm = *src; /* shallow */
    ...
    if (src->entities && src->entities_count > 0) {
        if (copy_entities(alloc, src->entities, src->entities_count, &wm->entities) != HU_OK) {
            xfree(alloc, wm, sizeof(*wm));
            return NULL;
        }
    } else {
    ...
    if (src->hyperedges && src->hyperedges_count > 0) {
        ...
        for (size_t i = 0; i < src->hyperedges_count; i++) {
            ...
            if (src->hyperedges[i].members && src->hyperedges[i].members_count > 0) {
                size_t mb = src->hyperedges[i].members_count * sizeof(*src->hyperedges[i].members);
                hu_hyperedge_member_t *mdup = xalloc(alloc, mb);
                if (!mdup)
                    goto clone_oom_in_hyperedges_;
...
clone_oom_in_hyperedges_:
    for (size_t i = 0; i < src->hyperedges_count; i++) {
        ...
        if (wm->hyperedges[i].provenance) {
            xfree(alloc, wm->hyperedges[i].provenance, strlen(wm->hyperedges[i].provenance) + 1);
        }
    }
...
clone_oom_after_changes_:
    if (wm->recent_changes) {
        xfree(alloc, wm->recent_changes, src->recent_changes_count * sizeof(*src->recent_changes));
    }
    for (size_t i = 0; i < src->relations_count; i++) {
        if (wm->relations && wm->relations[i].provenance) {
            xfree(alloc, wm->relations[i].provenance, wm->relations[i].provenance_len + 1);
...
hu_error_t hu_world_model_load_with_channel(hu_memory_facade_t *m, hu_allocator_t *alloc,
    ...
    if (cid_len == 0 || cid_len >= 64)
        return HU_ERR_INVALID_ARGUMENT;
    if (channel_len >= 32)
        return HU_ERR_INVALID_ARGUMENT;
    if (!channel)
        channel_len = 0; /* NULL channel == "any-channel" key */
    ...
    if (cache_init_locked_()) {
        ...
        if (slot) {
            ...
            if (slot->wm) {
                ...
                if (channel_len > 0)
                    memcpy(slot->channel, channel, channel_len);
