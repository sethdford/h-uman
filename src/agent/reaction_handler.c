...
hu_error_t hu_reaction_handler_handle_event(const hu_reaction_event_t *e) {
    ...
    /* Note: reactions are intentionally single-sided here (positive: only
     * `chosen` populated; negative: only `rejected`). These ARE written
     * to dpo_pairs but the iterator filter in hu_dpo_iterate_pairs skips
     * them at ORPO training time so they don't poison the corpus. The
     * long-term home is a separate signals table; see
