...
static void *spawn_thread(void *arg) {
    ...
    if (s->mode == HU_SPAWN_PERSISTENT) {
        ...
        if (s->inherit_verifier_graph) {
            hu_error_t ge =
                hu_agent_bind_sqlite_graph(ag, (struct hu_graph *)s->inherit_verifier_graph, a);
            if (ge != HU_OK)
                hu_log_warn("spawn", NULL, "spawn graph bind: %s", hu_error_string(ge));
        }
#endif
        if (s->inherit_metacognition_policy)
            hu_metacognition_apply_config(&ag->infra.metacognition,
                                          s->inherit_metacognition_policy);
