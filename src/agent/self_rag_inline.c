...
static size_t find_next_open_tag(const char *s, size_t len, size_t from, const char **tag_kind,
                                 size_t *open_end) {
    static const char *const k_names[] = {"retrieve", "critique", "refuse", NULL};
...
static size_t find_close_tag(const char *s, size_t len, size_t from, const char *name) {
...
static size_t append_clip(char *dst, size_t cap, size_t off, const char *src, size_t len) {
    if (off >= cap)
        return 0;
    size_t avail = cap - 1 - off; /* leave room for nul */
    size_t copy = len < avail ? len : avail;
    if (copy > 0)
        memcpy(dst + off, src, copy);
    dst[off + copy] = '\0';
    return copy;
}

/* Capture text into a fixed-size atomic_claim text buffer. */
static void capture_claim_text(hu_atomic_claim_t *c, const char *src, size_t len,
                               int64_t base_off) {
    memset(c, 0, sizeof(*c));
    size_t copy = len < sizeof(c->text) - 1 ? len : sizeof(c->text) - 1;
    if (copy > 0)
        memcpy(c->text, src, copy);
    c->text[copy] = '\0';
...
static bool inline_wm_entity_in_claim(const hu_world_model_t *wm, const char *claim,
                                      size_t claim_len) {
...
static const hu_negative_memory_t *inline_wm_negative_match(const hu_world_model_t *wm,
...
static void inline_score_critique(hu_allocator_t *alloc, hu_memory_facade_t *m,
                                  const char *contact_id, size_t cid_len, int64_t now,
                                  const hu_world_model_t *wm, hu_atomic_claim_t *c) {
    ...
    hu_error_t err =
        hu_response_verify(alloc, m, contact_id, cid_len, sentence, (size_t)n, &cfg, &report);
    ...
    const hu_negative_memory_t *neg_match = inline_wm_negative_match(wm, c->text, strlen(c->text));
    if (neg_match) {
        score = 0.0f;
    ...
    if (report.claims[0].receipt.source[0] && c->support.prov_count < 4) {
        hu_provenance_atom_t *p = &c->support.prov[c->support.prov_count++];
        snprintf(p->source, sizeof(p->source), "%s", report.claims[0].receipt.source);
...
static void inline_score_retrieve(hu_allocator_t *alloc, hu_memory_facade_t *m,
                                  const char *contact_id, size_t cid_len, const char *query,
    ...
    if (query && query[0] && n > 0) {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (hu_crag_grade_document(alloc, query, query_len, r->context, r->context_len,
                                       &grade) != HU_OK)
                continue;
    ...
    c->support =
        hu_belief_init(score, relevant_count > 0 ? "inline-probe-graded" : "inline-probe", now);
...
static hu_error_t inline_verify(void *vctx, hu_allocator_t *alloc, const hu_self_rag_request_t *req,
    ...
    if (req->mode == HU_VERIFY_OFF) {
        resp->outcome = HU_SELF_RAG_SUPPORTED;
        snprintf(resp->modified_draft, sizeof(resp->modified_draft), "%.*s", (int)req->draft_len,
                 req->draft);
        return HU_OK;
    }
    ...
    while (i < req->draft_len) {
        ...
        size_t open_end = 0;
        size_t open_at = find_next_open_tag(req->draft, req->draft_len, i, &kind, &open_end);
        if (open_at == (size_t)-1) {
            /* No more tags; emit the rest verbatim. */
            vis_off +=
                append_clip(visible, sizeof(visible), vis_off, req->draft + i, req->draft_len - i);
            break;
        }
        /* Emit text up to the tag. */
        vis_off += append_clip(visible, sizeof(visible), vis_off, req->draft + i, open_at - i);

        size_t close_at = find_close_tag(req->draft, req->draft_len, open_end, kind);
        if (close_at == (size_t)-1) {
            ...
            vis_off += append_clip(visible, sizeof(visible), vis_off, req->draft + open_at,
        ...
        if (strcmp(kind, "refuse") == 0) {
            ...
            if (content_len > 0) {
                ...
                hu_self_rag_render_refusal(HU_REFUSAL_POLICY, resp->refusal_text,
        ...
        if (resp->claims_count < sizeof(resp->claims) / sizeof(resp->claims[0])) {
            hu_atomic_claim_t *c = &resp->claims[resp->claims_count++];
            capture_claim_text(c, content_start, content_len, (int64_t)open_at);
            ...
            if (strcmp(kind, "critique") == 0) {
                inline_score_critique(alloc, ctx->m, req->contact_id, req->contact_id_len, now,
                                      req->wm, c);
            } else if (strcmp(kind, "retrieve") == 0) {
                inline_score_retrieve(alloc, ctx->m, req->contact_id, req->contact_id_len, c->text,
                                      now, c);
            } else {
        ...
        if (strcmp(kind, "critique") == 0) {
            vis_off += append_clip(visible, sizeof(visible), vis_off, content_start, content_len);
        }
    ...
    if (req->mode == HU_VERIFY_STRICT && req->abstain_threshold > 0.0f && resp->claims_count > 0) {
        size_t fabricated = 0;
    ...
    snprintf(resp->modified_draft, sizeof(resp->modified_draft), "%s", visible);
    if (vis_off != req->draft_len || memcmp(visible, req->draft, vis_off) != 0) {
        resp->draft_modified = true;
...
static bool flush_buffer_to_original(hu_self_rag_stream_ctx_t *ctx, const char *data, size_t len) {
...
hu_error_t hu_self_rag_stream_wrap(hu_self_rag_stream_ctx_t *ctx, hu_stream_callback_t original_cb,
                                   void *original_ctx, hu_memory_facade_t *memory,
        ...
        if (ctx->token_buf_len > 0) {
            ...
            if (complete >= 0) {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    ctx->critique_triggered = true;
                    break;
                case 2:
                    ctx->refuse_triggered = true;
                    break;
                }
...
hu_error_t hu_self_rag_stream_directive_append(hu_allocator_t *alloc, char **system_prompt,
    ...
    char *new_buf = (char *)alloc->realloc(alloc->ctx, *system_prompt, old_len + 1, new_len + 1);
    if (!new_buf)
...
hu_error_t hu_self_rag_inline(hu_memory_facade_t *m, hu_provider_t *chat, hu_self_rag_t *out) {
    if (!out)
        return HU_ERR_INVALID_ARGUMENT;
    (void)chat;
