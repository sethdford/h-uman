    ...
    char **out, size_t *out_len)
{
    ...
    size_t pos = 0;
    memcpy(buf + pos, static_prompt, static_prompt_len); pos += static_prompt_len;
    memcpy(buf + pos, "## Memory Context\n\n", mem_header_len); pos += mem_header_len;
    memcpy(buf + pos, memory_context, memory_context_len); pos += memory_context_len;
    memcpy(buf + pos, "\n\n", 2); pos += 2;
    buf[pos] = '\0';
