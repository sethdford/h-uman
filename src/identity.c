...
void sc_identity_build_unified(sc_identity_t *identity, const char *channel, const char *account_id,
                               const char *user_id) {
    if (!identity)
        return;
    ...
    if (channel) {
        ...
        if (n >= SC_IDENTITY_CHANNEL_LEN)
            n = SC_IDENTITY_CHANNEL_LEN - 1;
        memcpy(identity->channel, channel, n);
        identity->channel[n] = '\0';
    }
    if (user_id) {
        size_t n = strlen(user_id);
        if (n >= SC_IDENTITY_USER_LEN)
            n = SC_IDENTITY_USER_LEN - 1;
    ...
    if (channel && user_id) {
        ...
        if (account_id && *account_id) {
            if (pos + 1 < SC_IDENTITY_UNIFIED_LEN)
...
bool sc_identity_has_permission(sc_permission_level_t have, sc_permission_level_t required) {
