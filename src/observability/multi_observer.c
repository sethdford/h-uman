...
sc_observer_t sc_multi_observer_create(sc_allocator_t *alloc, const sc_observer_t *observers,
                                       size_t count) {
    if (!alloc)
    ...
    sc_multi_observer_ctx_t *ctx =
        (sc_multi_observer_ctx_t *)alloc->alloc(alloc->ctx, sizeof(sc_multi_observer_ctx_t));
    if (!ctx)
        return (sc_observer_t){.ctx = NULL, .vtable = NULL};
