...
hu_error_t hu_life_thread_query_active_sql(char *buf, size_t cap, size_t *out_len) {
    if (!buf || !out_len)
        return HU_ERR_INVALID_ARGUMENT;
    const char *sql = "SELECT id, thread, timestamp FROM life_threads WHERE active=1 "
...
hu_error_t hu_narration_event_record(sqlite3 *db, const char *event_type, const char *description,
    ...
    /* Legacy entrypoint: no source_contact_id (empty string → channel-wide).
     * Forwards to the P3-4 scoped record-fn. */
    return hu_narration_event_record_for(db, "", event_type, description, shareability_score, now);
}
...
hu_error_t hu_narration_event_record_for(sqlite3 *db, const char *source_contact_id,
    ...
    const char *sql = "INSERT INTO life_narration_events (event_type, description, "
                      "shareability_score, generated_at, source_contact_id) "
                      "VALUES (?, ?, ?, ?, ?)";
    int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK)
        return HU_ERR_INTERNAL;
    ...
    sqlite3_bind_int64(stmt, 4, now);
    sqlite3_bind_text(stmt, 5, source_contact_id ? source_contact_id : "", -1, SQLITE_STATIC);
    rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    return (rc == SQLITE_DONE) ? HU_OK : HU_ERR_INTERNAL;
}
...
int hu_narration_events_unsent(sqlite3 *db, float min_shareability, const char *target_contact_id,
    ...
    if (!db || !out_ids || max_out <= 0)
        return 0;
    /* 2026-05-16 P3-4: scope by source_contact_id.
     * - NULL or empty target → only return events with empty source (conservative;
     *   never leak per-contact events into a "no contact" query).
     * - Non-empty target X → return events where source is '' (channel-wide /
     *   legacy) OR exactly X.  Equality match in SQL avoids the prefix-match
     *   class of bug we hit elsewhere (P3-10). */
    sqlite3_stmt *stmt = NULL;
    const char *sql = "SELECT id FROM life_narration_events "
                      "WHERE shared_at IS NULL "
                      "  AND shareability_score >= ? "
                      "  AND (source_contact_id = '' OR source_contact_id = ?) "
                      "ORDER BY generated_at DESC LIMIT ?";
    int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK)
        return 0;
    sqlite3_bind_double(stmt, 1, (double)min_shareability);
    sqlite3_bind_text(stmt, 2, target_contact_id ? target_contact_id : "", -1, SQLITE_STATIC);
