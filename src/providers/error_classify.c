...
static bool contains_fold(const char *haystack, size_t hlen, const char *needle, size_t nlen) {
    ...
    for (size_t i = 0; i + nlen <= hlen; i++) {
        ...
        for (size_t j = 0; j < nlen && match; j++)
            match =
                (to_lower((unsigned char)haystack[i + j]) == to_lower((unsigned char)needle[j]));
        if (match)
            return true;
    }
    return false;
...
bool sc_error_is_context_exhausted(const char *msg, size_t msg_len) {
    ...
    if (has_context &&
        (contains_fold(msg, check_len, "length", 6) ||
         contains_fold(msg, check_len, "maximum", 7) ||
         contains_fold(msg, check_len, "window", 6) || contains_fold(msg, check_len, "exceed", 6)))
        return true;
    if (has_token &&
        (contains_fold(msg, check_len, "limit", 5) ||
         contains_fold(msg, check_len, "too many", 8) ||
         contains_fold(msg, check_len, "maximum", 7) || contains_fold(msg, check_len, "exceed", 6)))
        return true;
...
bool sc_error_is_rate_limited(const char *msg, size_t msg_len) {
    ...
    return contains_fold(msg, check_len, "429", 3) &&
           (contains_fold(msg, check_len, "rate", 4) || contains_fold(msg, check_len, "limit", 5) ||
            contains_fold(msg, check_len, "too many", 8));
}
