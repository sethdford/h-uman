...
static hu_weakness_type_t classify_by_category(const char *cat) {
    ...
    if (hu_str_contains_ci(cat, len, "math", 4) || hu_str_contains_ci(cat, len, "reason", 6) ||
        hu_str_contains_ci(cat, len, "logic", 5) || hu_str_contains_ci(cat, len, "arithmetic", 10))
        return HU_WEAKNESS_REASONING;
    if (hu_str_contains_ci(cat, len, "knowledge", 9) || hu_str_contains_ci(cat, len, "fact", 4) ||
        hu_str_contains_ci(cat, len, "trivia", 6) || hu_str_contains_ci(cat, len, "geo", 3) ||
        hu_str_contains_ci(cat, len, "history", 7) || hu_str_contains_ci(cat, len, "science", 7))
        return HU_WEAKNESS_KNOWLEDGE;
    if (hu_str_contains_ci(cat, len, "tool", 4) || hu_str_contains_ci(cat, len, "function", 8) ||
        hu_str_contains_ci(cat, len, "api", 3) || hu_str_contains_ci(cat, len, "code", 4))
        return HU_WEAKNESS_TOOL_USE;
    if (hu_str_contains_ci(cat, len, "format", 6) || hu_str_contains_ci(cat, len, "output", 6) ||
...
static hu_weakness_type_t classify_by_output(const char *actual, size_t actual_len,
    ...
    if (expected_len > 0 && actual_len > 0) {
        ...
        for (size_t i = 0; i < expected_len; i++)
            if (isdigit((unsigned char)expected[i])) {
                exp_has_digit = 1;
                break;
            }
        for (size_t i = 0; i < actual_len; i++)
            if (isdigit((unsigned char)actual[i])) {
                act_has_digit = 1;
                break;
            }
...
static void build_description(hu_weakness_t *w, const hu_eval_result_t *r,
    ...
    int n = snprintf(w->description, sizeof(w->description),
                     ...
                     task ? task->id : r->task_id, task ? task->expected : "(unknown)",
                     r->actual_output ? r->actual_output : "(empty)");
    w->description_len =
        (n > 0 && (size_t)n < sizeof(w->description)) ? (size_t)n : sizeof(w->description) - 1;
}
...
static void build_fix(hu_weakness_t *w) {
    ...
    if (len >= sizeof(w->suggested_fix))
        len = sizeof(w->suggested_fix) - 1;
...
hu_error_t hu_weakness_analyze(hu_allocator_t *alloc, const hu_eval_run_t *run,
                               const hu_eval_suite_t *suite, hu_weakness_report_t *out) {
    if (!alloc || !run || !out)
    ...
    out->items = (hu_weakness_t *)alloc->alloc(alloc->ctx, fail_count * sizeof(hu_weakness_t));
    if (!out->items)
    ...
    for (size_t i = 0; i < run->results_count; i++) {
        ...
        if (cat_type == HU_WEAKNESS_UNKNOWN) {
            cat_type =
                classify_by_output(r->actual_output, r->actual_output_len,
                                   task ? task->expected : NULL, task ? task->expected_len : 0);
        }
        ...
        if (r->task_id) {
            ...
            if (tl >= sizeof(w->task_id))
                tl = sizeof(w->task_id) - 1;
        ...
        if (task && task->category) {
            ...
            if (cl >= sizeof(w->category))
                cl = sizeof(w->category) - 1;
