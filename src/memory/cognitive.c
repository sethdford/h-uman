...
/* The F65 opinion SQL builders were removed 2026-07-18 - the repo-backed
 * API in human/memory/opinions.h (opinions_repo_sqlite.c) is the one
 * opinion store. See human/memory/opinion_challenge.h for the pure
 * challenge detector. */
...
hu_error_t hu_chapters_insert_sql(const hu_life_chapter_t *ch, char *buf, size_t cap,
    ...
    escape_sql_string(ch->theme ? ch->theme : "", ch->theme_len, theme_esc, sizeof(theme_esc),
                      &te_len);
    escape_sql_string(ch->mood ? ch->mood : "", ch->mood_len, mood_esc, sizeof(mood_esc), &me_len);
    ...
    int n = snprintf(
        ...
        (int)te_len, theme_esc, (int)me_len, mood_esc, (unsigned long long)ch->started_at,
        (unsigned long long)ch->ended_at, (int)th_len, threads_esc, ch->active ? 1 : 0);
    if (n < 0 || (size_t)n >= cap)
        return HU_ERR_INVALID_ARGUMENT;
    *out_len = (size_t)n;
    return HU_OK;
...
hu_error_t hu_chapters_close_sql(int64_t id, uint64_t ended_at, char *buf, size_t cap,
    ...
    if (!buf || !out_len || cap < 128)
        return HU_ERR_INVALID_ARGUMENT;
    int n = snprintf(buf, cap, "UPDATE life_chapters SET ended_at=%llu, active=0 WHERE id=%lld",
...
hu_error_t hu_chapters_build_prompt(hu_allocator_t *alloc, const hu_life_chapter_t *chapters,
    ...
    *out_len = 0;
    ...
    for (size_t i = 0; i < count && pos < cap - 64; i++) {
        ...
        n = snprintf(buf + pos, cap - pos, "Theme: %.*s. Mood: %.*s. Key threads: %.*s.", (int)tl,
                     theme, (int)ml, mood, (int)thl, threads);
        if (n < 0 || (size_t)n >= cap - pos)
            break;
        pos += (size_t)n;
    }
...
hu_error_t hu_social_graph_insert_link_sql(const hu_social_link_t *link, char *buf, size_t cap,
    ...
    escape_sql_string(link->contact_a ? link->contact_a : "", link->contact_a_len, a_esc,
                      sizeof(a_esc), &ae_len);
    escape_sql_string(link->contact_b ? link->contact_b : "", link->contact_b_len, b_esc,
                      sizeof(b_esc), &be_len);
    ...
    int n = snprintf(
        ...
        "VALUES ('%.*s', '%.*s', %d, %f, '%.*s')",
        (int)ae_len, a_esc, (int)be_len, b_esc, (int)link->rel_type, link->closeness, (int)ce_len,
        ctx_esc);
    if (n < 0 || (size_t)n >= cap)
        return HU_ERR_INVALID_ARGUMENT;
    *out_len = (size_t)n;
