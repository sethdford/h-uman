/* src/memory/repos/mood_repo_sqlite.c
 * Mood persistence (F60): the place the mood_log SQL + the raw sqlite3 handle +
 * the in-memory mood cache live, per the memory repository pattern (see
 * boundary_repo_sqlite.c). The domain module src/persona/mood.c keeps only the
 * pure hu_mood_name mapping + hu_mood_build_directive and no longer includes
 * <sqlite3.h>. Registered UNCONDITIONALLY (the domain module is unconditional)
 * so get_current/set resolve in every build — real under HU_ENABLE_SQLITE,
 * NOT_SUPPORTED stubs otherwise. Callers pass an injected hu_memory_t* and are
 * unchanged. The name<->enum mapping is consumed from the domain module
 * (hu_mood_name) so the MOOD_NAMES table is defined once. */
#include "human/core/allocator.h"
#include "human/core/error.h"
#include "human/memory.h"
#include "human/persona/mood.h"
#include <string.h> /* memset — used by both the sqlite impl and the #else stub */
...
static hu_mood_enum_t parse_mood(const char *name) {
    ...
    for (int i = 0; i < (int)HU_MOOD_COUNT; i++) {
        if (strcmp(name, hu_mood_name((hu_mood_enum_t)i)) == 0)
...
hu_error_t hu_mood_set(hu_allocator_t *alloc, hu_memory_t *memory, hu_mood_enum_t mood,
    ...
    const char *mood_str = hu_mood_name(mood);
