/* src/memory/repos/emotional_moments_repo_sqlite.c
 * Emotional-moments persistence: the place the record/get_due/mark SQL + the
 * raw sqlite3 handle live, per the memory repository pattern (see
 * boundary_repo_sqlite.c). The domain module src/memory/emotional_moments.c
 * keeps ONLY the pure select_topic predicate and no longer includes
 * <sqlite3.h>. Registered UNCONDITIONALLY (the domain module is unconditional)
 * so these symbols resolve in every build — real under HU_ENABLE_SQLITE,
 * NOT_SUPPORTED stubs otherwise. Callers pass an injected hu_memory_t* and are
 * unchanged; the signatures are preserved. */
...
hu_error_t hu_emotional_moment_record(hu_allocator_t *alloc, hu_memory_t *memory,
    ...
    (void)alloc;
    if (!memory || !contact_id || contact_id_len == 0 || !topic || topic_len == 0 || !emotion ||
        emotion_len == 0)
...
hu_error_t hu_emotional_moment_get_due(hu_allocator_t *alloc, hu_memory_t *memory, int64_t now_ts,
    ...
    *out_count = 0;
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (count >= cap) {
            cap *= 2;
            hu_emotional_moment_t *n = (hu_emotional_moment_t *)alloc->alloc(
                alloc->ctx, cap * sizeof(hu_emotional_moment_t));
            if (!n) {
...
hu_error_t hu_emotional_moment_mark_followed_up(hu_memory_t *memory, int64_t id) {
    ...
    sqlite3_stmt *stmt = NULL;
    int rc = sqlite3_prepare_v2(db, "UPDATE emotional_moments SET followed_up=1 WHERE id=?", -1,
                                &stmt, NULL);
    if (rc != SQLITE_OK)
        return HU_ERR_MEMORY_BACKEND;
