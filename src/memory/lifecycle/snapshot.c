...
sc_error_t sc_memory_snapshot_export(sc_allocator_t *alloc, sc_memory_t *memory, const char *path,
                                     size_t path_len) {
...
sc_error_t sc_memory_snapshot_import(sc_allocator_t *alloc, sc_memory_t *memory, const char *path,
                                     size_t path_len) {
    if (!alloc || !memory || !memory->vtable || !path || path_len == 0)
        return SC_ERR_INVALID_ARGUMENT;
    /* Reject path traversal */
