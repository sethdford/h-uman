...
static hu_error_t parse_gemini_response(hu_allocator_t *alloc, const char *json_body,
    ...
    hu_json_value_t *emb = hu_json_object_get(root, "embedding");
...
static hu_error_t gemini_embed(void *ctx, hu_allocator_t *alloc, const char *text, size_t text_len,
    ...
    if (mlen >= (int)sizeof(model_prefix) || mlen < 0)
        goto fail;
    if (hu_json_append_key_value(&buf, "model", 5, model_prefix, (size_t)mlen) != HU_OK)
        goto fail;
    if (hu_json_buf_append_raw(&buf, ",\"content\":{\"parts\":[{\"text\":", 29) != HU_OK)
        goto fail;
    if (hu_json_append_string(&buf, text, text_len) != HU_OK)
        goto fail;
    if (hu_json_buf_append_raw(&buf, "}]}}", 4) != HU_OK)
        goto fail;
    ...
    hu_error_t err = hu_http_post_json(alloc, url, NULL, body, body_len, &resp);
    hu_json_buf_free(&buf);
    if (err != HU_OK)
        return err;
...
static const hu_embedding_provider_vtable_t gemini_vtable = {
    .embed = gemini_embed,
    .name = gemini_name,
...
hu_embedding_provider_t hu_embedding_gemini_create(hu_allocator_t *alloc, const char *api_key,
    ...
    memset(g, 0, sizeof(*g));
    g->alloc = alloc;
    g->base_url = hu_strdup(alloc, GEMINI_BASE_URL);
    g->api_key = hu_strdup(alloc, api_key ? api_key : "");
