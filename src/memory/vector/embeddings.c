...
void sc_embedding_provider_free(sc_allocator_t *alloc, sc_embedding_provider_result_t *out) {
    if (!alloc || !out)
        return;
    if (out->values) {
...
static sc_error_t noop_embed(void *ctx, sc_allocator_t *alloc, const char *text, size_t text_len,
