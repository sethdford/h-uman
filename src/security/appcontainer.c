 ...
 * to create a child in an AppContainer. On non-Windows, returns HU_ERR_NOT_SUPPORTED.
 *
...
static hu_error_t appcontainer_wrap(void *ctx, const char *const *argv, size_t argc,
...
static hu_error_t appcontainer_apply(void *ctx) {
#ifndef _WIN32
    (void)ctx;
    return HU_ERR_NOT_SUPPORTED;
#else
...
static const hu_sandbox_vtable_t appcontainer_vtable = {
...
void hu_appcontainer_sandbox_init(hu_appcontainer_ctx_t *ctx, const char *workspace_dir) {
    ...
    snprintf(ctx->app_name, sizeof(ctx->app_name), "human-sandbox");
}
