...
#else
    long r =
        syscall(SYS_landlock_create_ruleset, NULL, 0, (uint32_t)LANDLOCK_CREATE_RULESET_VERSION);
    return (r >= 1 && r <= 5) ? r : 0;
#endif
}
...
static hu_error_t landlock_apply(void *ctx) {
    ...
    uint64_t fs_access =
        ...
        LANDLOCK_ACCESS_FS_MAKE_BLOCK | LANDLOCK_ACCESS_FS_MAKE_SYM | LANDLOCK_ACCESS_FS_EXECUTE;
    if (abi >= 2)
    ...
    for (size_t i = 0; i < sizeof(rules) / sizeof(rules[0]); i++) {
        ...
        long ret = syscall(SYS_landlock_add_rule, ruleset_fd, LANDLOCK_RULE_PATH_BENEATH,
                           &path_beneath, 0);
        close(path_fd);
        if (ret < 0 && i < 2) {
            close(ruleset_fd);
