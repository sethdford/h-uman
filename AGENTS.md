...
- minimal binary size (~23147 KB release with LTO)
...
Current scale: **1933 source + header files, ~473K lines of C, ~268K lines of tests, 13,163 tests, 59 channels**.

Performance baseline (macOS aarch64, MinSizeRel+LTO):

| Metric                   | Measured       |
| ------------------------ | -------------- |
| Binary size              | ~23147 KB       |
| Text section             | 480 KB         |
...
2. **Binary size and memory are hard product constraints**
   ...
   - Current release binary: ~23147 KB (all features with LTO).
...
5. **All 13,163+ tests must pass at zero ASan errors**
