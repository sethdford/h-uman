...
- minimal binary size (~23304 KB release with LTO)
...
Current scale: **1953 source + header files, ~477K lines of C, ~272K lines of tests, 13,300 tests, 31 channels**.

Performance baseline (macOS aarch64, MinSizeRel+LTO):

| Metric                   | Measured       |
| ------------------------ | -------------- |
| Binary size              | ~23304 KB       |
| Text section             | 480 KB         |
...
2. **Binary size and memory are hard product constraints**
   ...
   - Current release binary: ~23304 KB (all features with LTO).
...
5. **All 13,300+ tests must pass at zero ASan errors**
