...
Pre-built binaries are available via **Homebrew** (`brew install --HEAD ./Formula/human.rb`), **Docker** (`docker pull ghcr.io/sethdford/human:latest`), **Nix** (`nix run github:sethdford/human`), and **Debian** (`.deb` from [Releases](https://github.com/sethdford/human/releases)). See the main [README](https://github.com/sethdford/human#installation) for details.
:::
