...
hu_error_t hu_governor_init(const hu_proactive_budget_config_t *config, hu_proactive_budget_t *out);
...
/* Check if any budget remains. NOTE (2026-05-25 fix): no longer const —
 * the function now calls apply_resets internally to handle the daily/
 * weekly rollover. Previously a daemon that hit daily_max yesterday
 * stayed permanently GATED_BUDGET because nothing ever reset the
 * counters at the boundary; only the mutating record_sent /
 * filter_by_priority calls applied resets, and the init_proposer's
 * has_budget→short-circuit path never reached those. */
