...
struct sc_vector_store_vtable {
    sc_error_t (*upsert)(void *ctx, sc_allocator_t *alloc, const char *id, size_t id_len,
                         const float *embedding, size_t dims, const char *metadata,
                         size_t metadata_len);
    sc_error_t (*search)(void *ctx, sc_allocator_t *alloc, const float *query_embedding,
