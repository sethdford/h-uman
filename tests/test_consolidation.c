...
static void consolidation_removes_duplicates(void) {
    ...
    sc_error_t err = mem.vtable->store(mem.ctx, "cons_dup_a", 10, dup, strlen(dup), &cat, NULL, 0);
...
static void similarity_null_a_returns_0(void) {
    uint32_t score = sc_similarity_score(NULL, 0, "hello", 5);
    SC_ASSERT_EQ(score, 0u);
}

static void similarity_null_b_returns_0(void) {
    uint32_t score = sc_similarity_score("hello", 5, NULL, 0);
    SC_ASSERT_EQ(score, 0u);
}

static void similarity_both_empty_returns_100(void) {
    uint32_t score = sc_similarity_score("", 0, "", 0);
    SC_ASSERT_EQ(score, 100u);
}
...
static void similarity_same_prefix_reasonable(void) {
    ...
    uint32_t score = sc_similarity_score(a, 11, b, 11);
    SC_ASSERT_TRUE(score >= 30u);
    SC_ASSERT_TRUE(score <= 80u);
}

static void consolidation_null_memory_returns_error(void) {
    sc_allocator_t alloc = sc_system_allocator();
    sc_consolidation_config_t config = SC_CONSOLIDATION_DEFAULTS;
    sc_error_t err = sc_memory_consolidate(&alloc, NULL, &config);
...
#endif
    SC_RUN_TEST(similarity_null_a_returns_0);
    SC_RUN_TEST(similarity_null_b_returns_0);
    SC_RUN_TEST(similarity_both_empty_returns_100);
    SC_RUN_TEST(similarity_same_prefix_reasonable);
    SC_RUN_TEST(consolidation_null_memory_returns_error);
}
