...
static void test_none_memory_store(void) {
    ...
    hu_error_t err = mem.vtable->store(mem.ctx, "key", 3, "content", 7, &cat, NULL, 0);
...
static void test_none_memory_recall_empty(void) {
    ...
    size_t count = 0;
    hu_error_t err = mem.vtable->recall(mem.ctx, &alloc, "query", 5, 10, NULL, 0, &out, &count);
    HU_ASSERT_EQ(err, HU_OK);
    HU_ASSERT_NULL(out);
...
static void test_sqlite_memory_get(void) {
    ...
    bool found = false;
    hu_error_t err = mem.vtable->get(mem.ctx, &alloc, "k1", 2, &entry, &found);
    HU_ASSERT_EQ(err, HU_OK);
    HU_ASSERT_TRUE(found);
    HU_ASSERT_STR_EQ(entry.key, "k1");
...
static void test_sqlite_memory_forget(void) {
    ...
    bool deleted = false;
    hu_error_t err = mem.vtable->forget(mem.ctx, "forget_me", 9, &deleted);
    HU_ASSERT_EQ(err, HU_OK);
...
static void test_sqlite_memory_store_recall_list_forget_cycle(void) {
    ...
    size_t list_count = 0;
    hu_error_t err = mem.vtable->list(mem.ctx, &alloc, NULL, NULL, 0, &list_out, &list_count);
    HU_ASSERT_EQ(err, HU_OK);
    HU_ASSERT(list_count >= 1u);
    if (list_out) {
...
static void test_api_memory_create(void) {
    hu_allocator_t alloc = hu_system_allocator();
    hu_memory_t mem = hu_api_memory_create(&alloc, "https://example.com/api", "test-key", 5000);
    HU_ASSERT_NOT_NULL(mem.ctx);
    HU_ASSERT_NOT_NULL(mem.vtable);
    HU_ASSERT_STR_EQ(mem.vtable->name(mem.ctx), "api");
    mem.vtable->deinit(mem.ctx);
}

static void test_api_memory_store_without_server(void) {
    hu_allocator_t alloc = hu_system_allocator();
    hu_memory_t mem = hu_api_memory_create(&alloc, "https://example.com/api", "test-key", 100);
    hu_memory_category_t cat = {.tag = HU_MEMORY_CATEGORY_CORE};
    hu_error_t err = mem.vtable->store(mem.ctx, "key", 3, "val", 3, &cat, NULL, 0);
    /* HU_IS_TEST stub returns HU_OK */
    HU_ASSERT_EQ(err, HU_OK);
    mem.vtable->deinit(mem.ctx);
}
...
static void test_api_memory_health_check(void) {
    ...
    /* HU_IS_TEST stub returns true */
    HU_ASSERT(ok);
    mem.vtable->deinit(mem.ctx);
}

void run_memory_tests(void) {
    HU_TEST_SUITE("memory");
    HU_RUN_TEST(test_none_memory_create);
    HU_RUN_TEST(test_none_memory_store);
    HU_RUN_TEST(test_none_memory_recall_empty);
    HU_RUN_TEST(test_none_memory_count);
    HU_RUN_TEST(test_none_memory_health_check);
#ifdef HU_ENABLE_SQLITE
    HU_RUN_TEST(test_sqlite_memory_store_recall);
    HU_RUN_TEST(test_sqlite_memory_get);
    HU_RUN_TEST(test_sqlite_memory_forget);
    HU_RUN_TEST(test_sqlite_memory_store_recall_list_forget_cycle);
    HU_RUN_TEST(test_sqlite_memory_list_ordering);
    HU_RUN_TEST(test_sqlite_memory_session_scoped);
#endif

    HU_TEST_SUITE("memory — api engine");
    HU_RUN_TEST(test_api_memory_create);
    HU_RUN_TEST(test_api_memory_store_without_server);
    HU_RUN_TEST(test_api_memory_health_check);
}
