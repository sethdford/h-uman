...
/* 2026-07-13 — the SAME meta-text-echo class from a DIFFERENT subsystem
 * (GVR verify/revise, src/agent/gvr.c). Five reached real contacts:
 * "please revise the response to ensure it maintains a professional and
 * formal tone", "based on what I know, Hey!", + NEEDS_RETRY x3. */
static void g10_rejects_gvr_and_meta_instruction_echoes(void) {
    g10_expect_reject(
        "please revise the response to ensure it maintains a professional and formal tone");
    g10_expect_reject("based on what I know, Hey! What's up?");
    g10_expect_reject("You are a revision assistant. Produce a corrected response.");
    g10_expect_reject("FAIL: the response to verify is incomplete");
}

static void g10_passes_natural_uses_of_revise_and_know(void) {
    /* Content-bearing near-boundary phrases must survive — no meta shape. */
    g10_expect_pass("based on what I know the market is soft right now");
    g10_expect_pass("can you revise the offer and send it back?");
    g10_expect_pass("let's keep it professional but warm");
}
...
void run_response_guard_tests(void) {
    ...
    HU_RUN_TEST(g10_rejects_gvr_and_meta_instruction_echoes);
    HU_RUN_TEST(g10_passes_natural_uses_of_revise_and_know);
