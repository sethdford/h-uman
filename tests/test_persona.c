...
static void test_persona_full_round_trip(void) {
    ...
    char *sys = NULL;
    size_t sys_len = 0;
    err = hu_prompt_build_system(&alloc, &cfg, NULL, NULL, &sys, &sys_len);
    HU_ASSERT_EQ(err, HU_OK);
...
static void test_lean_prompt_size_under_5kb(void) {
    ...
    char *sp = NULL;
    size_t sp_len = 0;
    err = hu_prompt_build_system(&alloc, &cfg, NULL, NULL, &sp, &sp_len);
    HU_ASSERT_EQ(err, HU_OK);
...
static void test_lean_prompt_full_pipeline_sp_len(void) {
    ...
    err = hu_prompt_build_system(&alloc, &lean_cfg, NULL, NULL, &lean_sp, &lean_sp_len);
    HU_ASSERT_EQ(err, HU_OK);

    char *full_sp = NULL;
    size_t full_sp_len = 0;
    err = hu_prompt_build_system(&alloc, &full_cfg, NULL, NULL, &full_sp, &full_sp_len);
    HU_ASSERT_EQ(err, HU_OK);
