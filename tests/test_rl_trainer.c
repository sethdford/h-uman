 ...
 *   1. HUML backend returns a valid vtable. DISABLED — re-enable after
 *      Task 4 implements hu_dpo_real_huml_create; today the stub
 *      returns HU_ERR_NOT_SUPPORTED, which fails the assertion.
 ...
 *      task brief).
 *   3. AUTO backend falls through to a working backend. DISABLED —
 *      re-enable after Task 4 OR Task 6 implements at least one real
 *      backend; this test asserts `err == HU_OK` from the factory,
 *      which cannot hold while both backends are stubs returning
 *      HU_ERR_NOT_SUPPORTED. The plan-step-7 claim that this test
 *      passes at Task-1 time is inconsistent with the plan-step-5
 *      stub behaviour; documented as a deviation in the Task 1
 *      commit message.
 *
...
#if 0 /* Re-enable after Task 4 implements hu_dpo_real_huml_create */
...
void run_rl_trainer_tests(void) {
     ...
     *   Re-enable after Task 4 implements hu_dpo_real_huml_create.
     *   The stub currently returns HU_ERR_NOT_SUPPORTED, which would
     *   fail this test's first assertion. */
     ...
     *   Re-enable after Task 4 OR Task 6 implements a real backend.
     *   The factory currently returns HU_ERR_NOT_SUPPORTED from every
     *   backend stub, which fails this test's HU_OK assertion. */
}
