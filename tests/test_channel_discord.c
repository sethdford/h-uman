/* G3 regression tests: Discord channel ↔ hu_typing_send wiring.
 *
 ...
 *   4. hu_discord_set_persona(NULL) is a safe no-op
 */
...
static void test_discord_typing_null_persona_sends_directly(void) {
    hu_allocator_t alloc = hu_system_allocator();
    hu_channel_t   ch;
    HU_ASSERT_EQ(hu_discord_create(&alloc, "Bot-token", 9, &ch), HU_OK);
    hu_discord_set_persona(&ch, NULL);

    prime_typing_buffer();

    hu_error_t err = ch.vtable->send(ch.ctx, "123456789", 9, "hello", 5, NULL, 0);
    ...
    size_t msg_len = 0;
    const char *msg = hu_discord_test_get_last_message(&ch, &msg_len);
...
static void test_discord_typing_with_persona_calls_typing_send(void) {
    hu_allocator_t alloc = hu_system_allocator();
    hu_channel_t   ch;
    HU_ASSERT_EQ(hu_discord_create(&alloc, "Bot-token", 9, &ch), HU_OK);
    memset(&s_test_persona, 0, sizeof(s_test_persona));
    hu_discord_set_persona(&ch, &s_test_persona);

    prime_typing_buffer();

    hu_error_t err = ch.vtable->send(ch.ctx, "123456789", 9, "hello world", 11, NULL, 0);
    HU_ASSERT_EQ(err, HU_OK);
    ...
    /* Discord supports start/stop_typing → schedule has START_TYPING. */
...
static void test_discord_typing_null_persona_delivers_message(void) {
    hu_allocator_t alloc = hu_system_allocator();
    hu_channel_t   ch;
    HU_ASSERT_EQ(hu_discord_create(&alloc, "Bot-token", 9, &ch), HU_OK);

    hu_error_t err = ch.vtable->send(ch.ctx, "123456789", 9, "test message", 12, NULL, 0);
    HU_ASSERT_EQ(err, HU_OK);
...
/* 4. hu_discord_set_persona with NULL / bad channel is a safe no-op. */
static void test_discord_set_persona_handles_null_inputs(void) {
    hu_discord_set_persona(NULL, NULL);
    hu_discord_set_persona(NULL, (void *)0x1);
...
void run_channel_discord_tests(void) {
    HU_TEST_SUITE("Channel Discord Typing");
    HU_RUN_TEST(test_discord_typing_null_persona_sends_directly);
    HU_RUN_TEST(test_discord_typing_with_persona_calls_typing_send);
    HU_RUN_TEST(test_discord_typing_null_persona_delivers_message);
    HU_RUN_TEST(test_discord_set_persona_handles_null_inputs);
}
