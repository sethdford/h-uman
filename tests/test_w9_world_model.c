...
/* 2026-05-16 P3-8 regression: contact matching in hu_world_model_merge_persona
 * must use stable contact_id ONLY.  The previous case-insensitive display-name
 * fallback (strcasecmp(c->name, wm->contact_id) == 0) routed contact A's
 * persona profile — including allowed_behaviors — into a `wm` keyed by another
 * contact's stable id that happened to spell the first contact's display name
 * (e.g. Slack handle "betty" matching display name "Betty").
 *
 * Set up two contacts where one's display name equals the other's contact_id.
 * Verify that merging persona with the second contact's id does NOT pick up
 * the first contact's allowed_behaviors. */
static void test_w9_p3_8_world_model_strict_contact_id_no_display_name_fallback(void) {
#ifdef HU_ENABLE_SQLITE
    hu_graph_t *g = NULL;
    hu_memory_facade_t *m = NULL;
    open_facade_(&g, &m);
    ...
    hu_world_model_t *wm = NULL;
    HU_ASSERT_EQ(hu_world_model_build(m, A(), "betty", 5, 1735690000000LL, &wm), HU_OK);

    /* Contact A: display name "Betty", contact_id "betty_aunt".  Has an
     * allowed_behavior "tease about hiking" that ONLY A is supposed to expose. */
    hu_persona_t persona;
    hu_persona_overlay_t overlay;
    build_minimal_persona_(&persona, &overlay);

    static const char *aunt_allowed[] = {"tease about hiking"};
    hu_contact_profile_t aunt;
    memset(&aunt, 0, sizeof(aunt));
    aunt.contact_id = (char *)"betty_aunt";
    aunt.name = (char *)"Betty";
    aunt.allowed_behaviors = (char **)aunt_allowed;
    aunt.allowed_behaviors_count = 1;

    /* Contact B: contact_id "betty" (collides with A's display name in the
     * old strcasecmp fallback).  No allowed_behaviors. */
    hu_contact_profile_t friend_b;
    memset(&friend_b, 0, sizeof(friend_b));
    friend_b.contact_id = (char *)"betty";
    friend_b.name = (char *)"Beth-from-work";

    hu_contact_profile_t contacts[2] = {aunt, friend_b};
    persona.contacts = contacts;
    persona.contacts_count = 2;

    hu_world_model_merge_persona(wm, &persona, "slack", 5, NULL, 0);

    /* Contact B's wm must NOT see the aunt's allowed_behavior, because the
     * stable contact_ids ("betty_aunt" vs "betty") are different. */
    HU_ASSERT_NULL(strstr(wm->tom.user_expects_we_can, "tease about hiking"));
...
#ifdef HU_ENABLE_SQLITE
    HU_RUN_TEST(test_w9_p3_8_world_model_strict_contact_id_no_display_name_fallback);
