...
/* 2026-09-04: hu_app_teardown detached the semantic index from whatever
 * engine bootstrap had built; in the full suite that was the MARKDOWN engine
 * (sqlite path unavailable), and the setter cast its small ctx to the sqlite
 * struct and wrote two pointers past it (ASan heap-use-after-free). The
 * setter and getter must be no-ops on any engine that is not "sqlite", like
 * hu_sqlite_memory_get_db and hu_sqlite_memory_get_session_store already are. */
