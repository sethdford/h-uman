...
# ── Distress replies: how the user answers a sad / frustrated text ───────
#
# The 2026-09-12 twin failure was register, not label: to "😓" the model
# produced "I'm sorry to hear that. How can I help you with this…" and, on
# retry, "I understand this is frustrating. How can I help you…". The user's
# own replies to the same kind of inbound (chat.db, 120 days) are short and
# never the support register. This axis measures that directly, with no
...
SUPPORT_SCAFFOLDS = re.compile(
    r"(sorry to hear|i understand (this|that|how|you)|how can i help you|that must be|"
    r"i'?m here for you|i am here for you|that sounds (really|so) (hard|tough|frustrating)|"
    r"i'?m sorry you'?re)",
    re.I,
)
DISTRESS_REPLY_WINDOW_S = 30 * 60
DISTRESS_MIN_N = 10  # below this the card omits the axis and the prompt says nothing
...
def distress_stats(pairs) -> dict:
    """n, median reply length, support-scaffold rate. Empty when n == 0."""
    n = len(pairs)
    if n == 0:
        return {"n": 0}
    lengths = sorted(len(r) for _, r in pairs)
    scaffolds = sum(1 for _, r in pairs if SUPPORT_SCAFFOLDS.search(r))
    return {
        "n": n,
        "median_chars": lengths[n // 2],
        "scaffold_rate": scaffolds / n,
        "window_seconds": DISTRESS_REPLY_WINDOW_S,
    }
