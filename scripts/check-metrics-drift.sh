...
# Lines of C in src/ + include/, rounded to the nearest K, to match the docs'
# "~NNNK lines of C" phrasing (which includes public headers — same scope as
# update-stats.sh C_LINES_RAW, the writer).
...
for doc in "${KEY_DOCS[@]}"; do
  ...
  # Lines of C (src/ + include/), in K. Pattern is the specific "NNNK lines of C" phrasing so
